...
export default {
  ...
  methods: {
    ...
    handleDetailClick(params) {
      ...
      this.detailPlan = params.row

      this.detailPlanEdite = {
        name: this.detailPlan.name
      }

      // 生成值班计划详情 table 的数据
      // TODO 放到 factory 中
      this.detailColumns = params.row.templateDetail.map((item, index) => {
        return {
      ...
      this.detailColumns.unshift({
        title: '今日日期',
        minWidth: 100,
        key: 'date'
      })

      this.detailData = params.row.detail.map((item, index) => {
        let arr = []
        if (_.isArray(item.staffs)) {
          arr = item.staffs.map((staff, index) => {
            let arr = []
            if (_.isArray(staff)) {
              arr = staff.map((item, index) => {
                return item._id
              })
            }
            return arr
        ...
        if (this.detailDateTime === null) {
          this.detailDateTime = []
        }

        this.detailDateTime.push(item.date)
        arr.unshift(moment(item.date).format('YYYY-MM-DD'))
        return arr
      })
      ...
      personnelApi
        ...
        .then(res => {
          ...
          this.personnelList = list.map(item => {
            ...
            personnelListObj[obj.id] = obj.name
            return obj
          })
    ...
    confirmEditePlan() {
      let params = {
        ...
        startTime: new Date(this.newPlan.startTime).getTime(),
        endTime: new Date(this.newPlan.endTime).getTime(),
        template: this.newPlan.template,
        group: [],
        detail: this.newPlan.detail
      }

      params.group = this.newPlanOrder.map((item, index) => {
        if (item.org) {
          return {
            org: {
              _id: item.org.id
    ...
    closeEditePlan() {
      this.getTeam()
      this.modalVisibleEditePlan = false
      this.$refs['editePlan'].resetFields()
    },
