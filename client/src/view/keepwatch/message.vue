    ...
    <div class="bs-main flex-column ">
      <div class="main-handle flex-between topBar">
        ...
        <div class="handle-right">
          <Input v-model="searchData.title" placeholder="消息关键字" style="width: 150px"></Input>
          <Input v-model="searchData.sender" placeholder="发件人" style="width: 150px"></Input>
          <Select style="width:150px" @on-change="searchAllMesList" v-model="searchData.type">
            <Option v-for="item in allMessageList" :value="item.value" :key="item.value">{{ item.label }}</Option>
          </Select>
          <div>
...
export default {
  ...
  created() {
    ...
    this.getOrgTree(3)
    this.getMessageList({ limit: this.searchData.limit })
  },
  mounted() {
  ...
  methods: {
    ...
    replyMsg(data) {
      ...
      this.replyUser.userId = data.senderId._id
      this.modalShow = true
    },
    addMsgOk() {
      this.getMessageList(this.searchData)
