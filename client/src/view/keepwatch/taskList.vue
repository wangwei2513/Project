<template>
  <div class="bs-content">
    <div v-if="modal">
      <Modal v-model="modal" :title="modalTitle" @on-cancel="modalClose" :mask-closable="false">
        <Form ref="taskInfo" :rules="taskFormRole" :model="taskInfo" :label-width="80" label-position="left">
          ...
          <FormItem label="任务时间">
            <TimePicker :clearable="false" :disabled="taskInfo.taskType==1" format="HH:mm" @on-change="startTimeChange" :editable="false" width="150px" type="time" placeholder="开始时间" :value="taskInfo.taskType==1?'':taskInfo.startTime"></TimePicker>
            <span>-</span>
            <TimePicker :clearable="false" :disabled="taskInfo.taskType==1" @on-change="endTimeChange" :value="taskInfo.taskType==1?'':taskInfo.endTime" format="HH:mm" :editable="false" type="time" width="150px" placeholder="结束时间"></TimePicker>
          </FormItem>
...
export default {
  ...
  methods: {
    ...
    startTimeChange(time) {
      this.taskInfo.startTime = time
    },
    // 自由模式 修改结束时间
    endTimeChange(time) {
      this.taskInfo.endTime = time
    },
    // 提交巡更任务表单
    modalOk() {
