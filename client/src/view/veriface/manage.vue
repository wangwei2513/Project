  ...
  <div class="manage" v-resize="resizeFun">
    ...
    <aside>
      ...
      <Modal title="批量导入" v-model="showUpload" :mask-closable="false" @on-visible-change="closeUploadModel">
        <Form ref="batUploadForm" :label-width="85" label-position="left">
          <FormItem label="底库类型">
...
<script>
import { validateName, validateDesc, validateAge, validateIdentity } from './common/validate'
  ...
  methods: {
    ...
    saveContent() {
      if (this.showPeopleModel) {
        this.$refs.peopleValidate.validateField('name', valid => {
          if (!valid) {
    ...
    // 获取人员列表
    getPeopleData(groupId = this.groupId) {
      this.getManagUser({ page: this.pageInfo.cur, limit: this.pageInfo.limit, id: groupId })
        .then(res => {
