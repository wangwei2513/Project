...
export default {
  ...
  computed: {
    ...mapState({
      ...
      playingCameraList: state => state.veriface.playingCameraList,
  ...
  watch: {
    ...
    playingPluginIds(data) {
      ...
      let openIds = []
      ids.map(id => {
        this.openTemp.map((item, index) => {
          if (item && id === item.id) {
            this.setVerifacePlayingCameraList({index, data: item})
    ...
    strangerNumber() {
      this.realtimeNumber++
      this.realtimeNumber > 99999999 && (this.realtimeNumber = 0)
      this.$nextTick(() => {
    ...
    defenseAlarmNumber() {
      ...
      this.contrastNumber > 99999999 && (this.contrastNumber = 0)
  ...
  methods: {
    ...mapActions([
      ...
      'getMonitorList',
      'setDefaultSearch',
      'setVerifacePlayingCameraList',
      'setImageUrl',
      'setVerifaceContrastList'
    ]),
    ...mapMutations(['CHANGE_VERIFACE_OPENARR']),
    refreshOrg() {
    ...
    handlePreview(node) {
      ...
      // this.setVerifacePlayingCameraList({data: param, index: this.plugin.activedIndex})
