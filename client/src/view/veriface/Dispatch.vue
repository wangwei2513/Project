  ...
  <div class="dispatch" v-resize="resizeFun">
    ...
    <main class="dispatch-main" ref="peopleTable">
      ...
      <Modal class="dispatch-model" :title="modelTitle" v-model="addDispatch" :mask-closable="false" @on-visible-change="closeAddModel">
        <Tabs type="card" :animated="false" v-model="clickTab">
            <TabPane label="布控信息" name="布控信息" class="tab-pane">
              <div style="display:flex;">
                <Form ref="infoValidate" class="dispatch-form" :model="infoValidate" :rules="infoRuleValidate" label-position="left" :label-width="85" style="width:275px; margin-right:20px">
                  ...
                  <FormItem label="开始时间：" prop="startTime">
                    <Date-picker
                      type="datetime"
                      v-model="infoValidate.startTime"
                      :clearable="false"
                      :confirm="false"
                      :disabled="disabledTime"
                      :readonly="disabledTime"
                      placeholder="选择日期"
                      @on-change="startTimeSave"></Date-picker>
                  </FormItem>
                  <FormItem label="结束时间：" prop="endTime">
                    <Date-picker
                      type="datetime"
                      v-model="infoValidate.endTime"
                      placeholder="选择日期"
                      :clearable="false"
                      :readonly="disabledTime"
                      :disabled="disabledTime"
                      @on-change="endTimeSave"></Date-picker>
                  </FormItem>
                ...
                <bs-scroll ref="scroller" style="height: 448px; width: 100%; margin-left: 30px">
                  <Bsr-Tree :treeData="dispatchTree" :selectNode="selectNode" :showCheckbox="true" ref="bstree"
                    @on-expand="expand"
                    @handlechecked="checkedTree" style="flex:1;">
            ...
            <TabPane label="抓拍参数" name="抓拍参数" class="tab-pane">
...
export default {
  ...
  methods: {
    ...
    nextStep() {
      this.$refs.infoValidate.validateField('name', valid => {
        if (!valid) {
          this.$refs.infoValidate.validateField('groups', valid => {
            if (!valid) {
              if (!this.isTrueDate) {
                return this.errorMsg('开始时间不能大于结束时间')
              }
              this.clickTab = '抓拍参数'
              // const result = this.countRepeatPoint(this.points).length
              // if (result < 1) {
              //   this.clickTab = '抓拍参数'
              // } else {
              //   this.errorMsg('点位元素不能重复')
              // }
            }
          })
        }
    ...
    updateTask() {
      ...
      this.points = this.$lodash.cloneDeep(result.points)
    ...
    startTimeSave(val) {
      if (val === '') {
        this.infoValidate.startTime = new Date()
      }
      // 开始时间不能小于结束时间
    },
    endTimeSave(val) {
      ...
      nextMonth.setMonth(nextMonth.getMonth() + 1)
      if (val === '') {
        this.infoValidate.endTime = nextMonth
      }
    },
    // 表格选中
    selectTask(arr) {
    ...
    checkedTree(arr) {
      this.points = arr
      // const result = this.countRepeatPoint(arr)
      // if (result.length > 0) {
      //   this.errorMsg('点位元素不能重复')
      // }
    },
    countRepeatPoint(newArr, oldArr = this.allCheckedPoint) {
      if (newArr.length === 0) {
        return []
      }
      if (this.updateId) {
        const arr = newArr.filter(_id => {
          return !this.selectNode.includes(_id)
        })
        if (arr.length > 0) {
          return oldArr.filter(_id => {
