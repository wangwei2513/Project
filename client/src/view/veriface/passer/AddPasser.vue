        ...
        <div class='ivu-modal-content'>
          ...
          <div class='ivu-modal-body over clearfix'>
            <Form class="from-size" :model="passer" :rules="inforules" ref="passerValidate" inline :label-width="75">
                  ...
                  <Option v-for="(item,j) in baserary" :key="j" :value="item.label">{{item.value}}</Option>
								</Select>
							</FormItem>
							...
							<FormItem  label="年龄" prop="age">
								<Input-number :max="100" :min="1" v-model="passer.age" style="width:254px;" placeholder="请输入年龄"></Input-number>
								</Input>
							</FormItem>
...
<script>
import { validateName, validateAge, validateIdentity, validateDesc } from '../common/validate'
...
export default {
  ...
  methods: {
    ...
    addUser() {
      ...
      if (!this.passer.group) {
        this.warningMsg('请选择底库！')
        return
      }
      const passer = {
      ...
      this.$refs.passerValidate.validateField('name', valid => {
        if (!valid) {
