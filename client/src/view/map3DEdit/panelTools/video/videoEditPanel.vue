...
<template>
  <div class="panel">
    <div class="panel_top">
      <span class="top_title">视频点位</span>
      <div class="btn_group">
        <span title="预览" @click="preViewVideo()" class="top_button iconfont icon-videotape"></span>
        <span title="删除" @click="deleteVideoPoint()" class="top_button iconfont icon-delete"></span>
      </div>
    </div>
        ...
        <div class="videoForm">
          ...
          <Form ref="pointInfo" :rules="validateRule" :model="pointInfo" :label-width="75" label-position="left">
            <FormItem label="名称" prop="name">
              <Input v-model="pointInfo.name" :title="pointInfo.name" :maxlength="64" placeholder="" style="width:175px;"/>
            </FormItem>
            <FormItem label="厂商" prop="firm">
              <Input v-model="pointInfo.firm" :title="pointInfo.firm"  placeholder="" style="width:175px;" readonly disabled/>
            </FormItem>
    ...
    <div class="panel_foot">
      <Button type="ghost" @click="cancelSave('pointInfo')" style="margin-right: -3px" title="点击取消保存点位信息">取消</Button>
      <Button type="primary" @click="saveVideoPoint('pointInfo')" style="margin-left: 16px" title="点击保存点位信息">保存</Button>
    </div>
  </div>
</template>
<script>
import videoEditBase from './videoEditBase'
import pointEdit from '../pointEdit'
import contentWay from '../contentWay'

export default {
  name: 'videoEditPanel',
  mixins: [ pointEdit, videoEditBase ],
