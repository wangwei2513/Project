    ...
    <div class="body">
      <div class="body-bac">
        <div class="profession-box" v-if="mainMenu[0] && mainMenu[0].children && mainMenu[0].children.length">
          ...
          <div class="profession-content" align="center">
            <div class="profession-content-box" align="left">
              <div class="list-item" v-for="menu in mainMenu[0].children" :key="menu.tag">
        ...
        <div class="profession-box2" v-if="mainMenu[1] && mainMenu[1].children && mainMenu[1].children.length">
          ...
          <div class="profession-content" align="center">
            <div class="profession-content-box" align="left">
              <div class="list-item" v-for="nenukey in mainMenu[1].children" :key="nenukey.tag">
...
export default {
  ...
  methods: {
    ...mapActions(['setMenueList', 'logoutFun', 'getPlatform', 'getMainMenu']),
    checkmenue(item) {
      ...
      this.setMenueList(data).then(() => {
        if (item.tag === '/map/module') { // 地图应用
          let url = '/map/' + read('mapType')
          this.$router.replace({ path: url })
        } else if (item.tag === '/mapEdit') { // 地图编辑
          let url = '/mapEdit/' + read('mapType')
          this.$router.replace({ path: url })
        } else {
