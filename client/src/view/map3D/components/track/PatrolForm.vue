      ...
      <Form :rules="validateRules" ref="patrolForm" :model="patrolForm" :label-width="100" label-position="left">
        ...
        <div v-if="patrolForm.type === 'patrol'">
          <FormItem label="日期" prop="patrolDate">
            <DatePicker type="date" placeholder="日期" v-model="patrolForm.patrolDate" style="width: 170px; margin-right: 10px;"></DatePicker>
          </FormItem>
        ...
        <div v-if="patrolForm.type === 'history'">
          <FormItem  label="时间" prop="historyDate">
            <DatePicker type="date" placeholder="日期" v-model="patrolForm.historyDate" style="width: 170px; margin-right: 10px;"></DatePicker>
            <TimePicker format="HH:mm:ss" v-model="patrolForm.timeRange" type="timerange" placement="bottom-end" placeholder="选择时间范围" style="width: 170px"></TimePicker>
          </FormItem>
        </div>
      </Form>
    </div>
  ...
  data() {
    return {
      ...
      patrolForm: {
        ...
        patrolDate: '', // 巡更查询日期
        historyDate: '', // 历史查询日期
        timeRange: ['00:00:00', ''] // 历史轨迹查询时间范围
      },
      validateRules: { // 表单校验规则
        name: [{required: true, message: '请选择巡更人员', trigger: 'change'}],
        patrolDate: [{ required: true, validator: this.validatePatrolDate, trigger: 'change' }],
  ...
  methods: {
    ...
    handlePatrolTypeChange(patrolType) { // 处理巡更轨迹查询类型改变
      if (patrolType === 'patrol') { // 巡更轨迹
        delete this.validateRules.historyDate // 去掉历史查询时间范围的校验
        this.validateRules.patrolDate = [{ required: true, validator: this.validatePatrolDate, trigger: 'change' }] // 更新时间校验
        this.validateRules.taskID = [{ required: true, message: '请选择巡更任务', trigger: 'change' }] // 添加选取时间或选择巡更任务的校验
      } else if (patrolType === 'history') { // 历史轨迹
        delete this.validateRules.patrolDate // 去掉巡更日期校验
        delete this.validateRules.taskID // 去掉任务校验
        this.validateRules.historyDate = [{ required: true, validator: this.validateSearchRange, trigger: 'change' }] // 查询时间范围校验
      }
    },
    ...
    searchPatrolHistoryTrack(name) { // 查询巡更历史记录
      ...
      let start = this.patrolForm.timeRange[0]
      let end = this.patrolForm.timeRange[1]
    ...
    initSearchTime() { // 初始化查询时间
      ...
      this.patrolForm.timeRange = ['00:00:00', this.$moment().format('HH:mm:ss')]
    },
    validatePatrolDate(rule, value, callback) { // 校验巡更轨迹查询日期
      if (!value) {
        callback(new Error('请输入查询日期'))
      }
      callback()
    },
    validateSearchRange(rule, value, callback) { // 校验历史轨迹查询时间范围
      if (!value) {
        callback(new Error('请输入查询日期'))
      } else if (!this.patrolForm.timeRange[0] || !this.patrolForm.timeRange[1]) {
        callback(new Error('请输入查询时间范围'))
      }
      callback()
    }
  }
