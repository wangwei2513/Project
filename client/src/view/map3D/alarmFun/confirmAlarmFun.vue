...
export default {
  ...
  methods: {
    ...
    confirmAlarmData(obj) {
      ...
      if (pointIsouter) {
        this.alarmingEntities.forEach((item, index) => {
          if (item.id === id) {
            this.alarmingEntities.splice(index, 1)
            this.setAllAlarmEntities(this.alarmingEntities)
          }
        })
        if (this.is3DMapOuter) {
          let entity = utils.getEntity(id, this.$context)
          if (entity) {
            this.getPointModUrl(id, type, false).then(res => {
              console.log(res, 's3Url')
              entity.model.uri = res
            })
          }
        }
      } else {
  ...
  mounted() {
    this.alarmingEntities = JSON.parse(JSON.stringify(this.allAlarmEntities))
