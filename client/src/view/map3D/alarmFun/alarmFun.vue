  ...
  methods: {
            ...
            if (entity) {
              ...
              if (data.type === 'sentry_alarm') {
                ...
                this.getPointModUrl(pointObj.id, pointObj.type, true).then(res => {
                  entity.model.uri = res // 'http://192.168.22.51:8090/iserver/mode/test/baojingzhudong.gltf'
                })
              }
            } else {
    ...
    creatBuildingAlarmObj(data, type, alarmType) {
      ...
      if (!this.alarmObj[code]) {
        //  || (this.alarmObj[code][type] && !this.alarmObj[code][type].children)
        this.alarmObj[code] = {}
    ...
    receiveSinglePos(data) {
      ...
      if (this.is3DMapOuter) {
        if (this.is3DMapOuter && this.ready && data.point.lon && data.point.lat) {
          ...
          if (entity) {
            let pos = context.Cesium.Cartesian3.fromDegrees(
              Number(data.point.lon),
              Number(data.point.lat),
              Number(data.point.height)
            )
            entity.position = pos
          } else {
