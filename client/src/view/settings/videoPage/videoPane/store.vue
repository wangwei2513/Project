<template>
  <div class="store-box" ref="tableBox">
    <div class="manage-head">
      <label>存储服务器</label>
      <Select :disabled="!isDisable" class="select-style" @click.native="waringChange" v-model="resetData.storeServer" style="width:390px" placeholder="请选择">
        <Option v-for="item in serverStoreList" :value="item.server" @click.native="serverChange(item.server)" :key="item._id">{{ item.server }}</Option>
      </Select>
      <label>存储路径</label>
      <Select :disabled="!isDisable" class="select-style" @click.native="waringChange" v-model="resetData.storePath" style="width:200px" placeholder="请选择">
        <Option v-for="item in getStorePath" :value="item.path" @click.native="pathChange(item.path)" :key="item._id">{{ item.path }}</Option>
      </Select>
    ...
    <div class="table-box">
      <Table size="small" :height="tableHeight" :highlight-row="true" @on-selection-change="select" :columns="columns4" :data="storeList"></Table>
...
export default {
      ...
      columns4: [
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            var that = this
            var serverSelect = params.row.conf ? params.row.conf.server : ''
            let flagIndex = false
              ...
              h(
                'select',
                {
                  ...
                  on: {
                    change(e) {
                      let v = e.target.value
                      _this.storeList[indexParams].conf.path = v
                      for (let i = 0; i < _this.changeLIst.length; i++) {
                        _this.storeList[_this.changeLIst[i]]._checked = true
                      }
                    }
                  }
                },
  ...
  methods: {
    ...mapActions(['getStorageByTree', 'changeStorageByUser', 'getStorageServer', 'changeAllConfig', 'recordLog']),
    waringChange() {
    ...
    saveStore() {
      ...
      this.changeStorageByUser(saveChangeList)
        ...
        .catch(err => {
          this.loading = false
          console.log('save storerage err' + err)
    ...
    this.getStorageServer({ servername: 'ds' })
      .then(suc => {
        for (let i = 0; i < suc.data.length; i++) {
          this.serverStoreList.push({ _id: suc.data[i], server: suc.data[i] })
        }
      })
      .catch(err => {
