  ...
  <div class="info-style userInfo">
    ...
    <Modal class="creat-modal" :mask-closable="false" v-model="ukeyShow" width="550" title="生成Ukey" @on-cancel="ukeyCancel">
      ...
      <div class="list-style">
        <ul>
          <li class="li-active"  v-for="item in ukeyList" :key="item.name">
  ...
  methods: {
    ...
    testUkey() {
      this.monitorUkey().then(suc => {
        ...
        let data = {
          name: suc.id,
          status: suc.key !== '' ? '已授权' : '未授权'
        }
        this.ukeyList.push(data)
      }).catch(err => {
        console.log(err)
      })
    },
     ...
     */
    writeUkey() {
      this.programmerUkey().then(suc => {
        this.formItem.ukey = suc.key
        this.percentNum = 100
      }).catch(err => {
        console.log(err)
      })
