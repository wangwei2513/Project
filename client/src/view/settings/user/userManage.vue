...
export default {
  created() {
    if (this.customPower) { return this.$router.replace({ path: '/settings/user/custom' }) }
    if (this.rolePower) { return this.$router.replace({ path: '/settings/user/role' }) }
    if (this.safePower) { return this.$router.replace({ path: '/settings/user/safe' }) }
  },
  computed: {
    activeTab() {
    ...
    customPower() {
      return (
        this.$BShasPower('BS-SETTING-USER-CUSTOM') ||
        this.$BShasPower('BS-SETTING-USER-CUSTOM-PAGE')
      )
    },
    rolePower() {
      return (
        this.$BShasPower('BS-SETTING-USER-ROLE') ||
        this.$BShasPower('BS-SETTING-USER-ROLE-PAGE')
      )
    },
    safePower() {
