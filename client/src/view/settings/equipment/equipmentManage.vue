  ...
  <div class="equipment bs-content" v-resize="resize">
    ...
    <div class="bs-main">
      <div class="equipment-right" v-if="$BShasPower('BS-SETTING-EQUIPMENT-VIDEO') || $BShasPower('BS-SETTING-EQUIPMENT-ALARM') || $BShasPower('BS-SETTING-EQUIPMENT-DECODE') || $BShasPower('BS-SETTING-EQUIPMENT-FIRE')">
        <div class="equipment-right-device" :style="{height: deviceActiveTab === 6 ? `100%` : `calc(100% - ${parseInt(this.botHeight) + 10}px)`}">
          <div class="table-header">
            <TableTab @on-tab-click="deviceTabClick" :tabs="deviceTabs" :isCount="true"></TableTab>
            <div class="table-header-actions clear">
              <div class="actions-btn">
                <Button type="ghost" icon="plus" @click="deviceSingleAdd" :disabled="!thirdParty" v-if="($BShasPower('BS-SETTING-EQUIPMENT-VIDEO-MANAGE') && deviceActiveTab === 0) || ($BShasPower('BS-SETTING-EQUIPMENT-DECODE-MANAGE') && deviceActiveTab === 5) || ($BShasPower('BS-SETTING-EQUIPMENT-ALARM-MANAGE') && deviceActiveTab === 1) || ($BShasPower('BS-SETTING-EQUIPMENT-FIRE-MANAGE') && deviceActiveTab === 7)||deviceActiveTab === 9|| deviceActiveTab === 6">添加</Button>
                <Button type="ghost" icon="edit" @click="deviceEditModal" :disabled="!(isDeviceChecked && thirdParty)" v-if="($BShasPower('BS-SETTING-EQUIPMENT-VIDEO-MANAGE') && deviceActiveTab === 0) || ($BShasPower('BS-SETTING-EQUIPMENT-DECODE-MANAGE') && deviceActiveTab === 5) || ($BShasPower('BS-SETTING-EQUIPMENT-ALARM-MANAGE') && deviceActiveTab === 1) || ($BShasPower('BS-SETTING-EQUIPMENT-FIRE-MANAGE') && deviceActiveTab === 7)||deviceActiveTab === 9|| deviceActiveTab === 6">修改</Button>
                <Button type="ghost" icon="trash-a" @click="deviceDeleteOpen" :disabled="!(isDeviceChecked && thirdParty)" v-if="($BShasPower('BS-SETTING-EQUIPMENT-VIDEO-MANAGE') && deviceActiveTab === 0) || ($BShasPower('BS-SETTING-EQUIPMENT-DECODE-MANAGE') && deviceActiveTab === 5) || ($BShasPower('BS-SETTING-EQUIPMENT-ALARM-MANAGE') && deviceActiveTab === 1) || ($BShasPower('BS-SETTING-EQUIPMENT-FIRE-MANAGE') && deviceActiveTab === 7)||deviceActiveTab === 9|| deviceActiveTab === 6">删除</Button>
              ...
              <div class="actions-search">
                <Input placeholder="请输入设备名称、设备IP" style="width: 220px" v-model="filterKey" @on-change="seekDeviceData(1,true)"  @keyup.enter.native="seekDeviceData(1)">
    ...
    <Modal :mask-closable="false" v-model="deviceAddModal" width="500" :title="isEditDevice?'设备修改':'设备添加'" @onCancel="deviceInfoCancel">
      ...
      <div class="device-video-Modal" v-show="deviceActiveTab === 0">
        ...
        <div class="deivce-modal-form" v-show="deviceModalState.pageOneShow">
          <!-- 基础信息form -->
          <Form :model="deviceFormData" label-position="left" :label-width="100" ref="deviceVideOneForm" :rules="deviceFormRole" v-show="deviceFormData.manufacturer !== 'custom'">
            ...
            <Form-item label="产品系列" v-if="deviceFormData.manufacturer !== 'hikvision'">
            ...
            <Form-item label="数据端口" prop="dport">
          ...
          <!-- 自定义时候的form -->
          <Form :model="deviceFormData" label-position="left" :label-width="100" ref="deviceVideCustomForm" :rules="deviceFormRole" v-show="deviceFormData.manufacturer === 'custom'">
            <Form-item label="设备名称" prop="name">
        ...
        <div class="deivce-modal-form" v-show="deviceModalState.pageTwoShow">
          <Form label-position="left" :label-width="100" :model="deviceFormData" ref="deviceVideTwoForm" :rules="deviceFormRole">
            ...
            <!-- 适配器地址 巨安信厂商时显示-->
            <Form-item label="Adapter地址" prop="adapterIp" v-if="deviceFormData.manufacturer === 'juanxin'">
              <Bsipv4 v-model="deviceFormData.adapterIp" v-if="isSingleAdd || isEditDevice"></Bsipv4>
              <!-- <Bsipv4s v-model="deviceIps" v-show="!isSingleAdd && !isEditDevice"></Bsipv4s> -->
            </Form-item>
            <Form-item label="Adapter端口" prop="adapterPort" v-if="deviceFormData.manufacturer === 'juanxin'">
      ...
      <!-- 解码器设备弹出框-->
      <div class="device-decoder-Modal" v-show="deviceActiveTab === 5">
        <div class="device-modal-header">
          <Button-group>
            <Button :type="deviceModalState.pageOneShow ? 'primary' : 'ghost'" @click="modalPageClick('P1')">1 - 基础信息</Button>
            <Button :type="deviceModalState.pageTwoShow ? 'primary' : 'ghost'" @click="modalPageClick('P2')">2 - 高级信息</Button>
...
export default {
    ...
    return {
      ...
      fireEngineColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            let text = ''
            if (params.row.manufacturer === 'lida') {
              text = '利达'
            }
            return h('span', text)
          }
      ...
      deviceFormRole: {
        ...
        ipcount: [{ validator: deviceValidate.deIpcount, trigger: 'change' }],
        defenseicount: [{ validator: deviceValidate.deDefent, trigger: 'change' }],
        defenseocount: [{ validator: deviceValidate.deDefent, trigger: 'change' }],
        ...
        voicecount: [{ validator: deviceValidate.deDefent, trigger: 'change' }],
        mainStream: [
  ...
  watch: {
    ...
    'deviceFormData.type'(val, oldVal) {
      if (val === 'ipc') {
        ...
        if (val === 'ipc' && this.deviceFormData.manufacturer === 'bstar') {
          ...
          this.deviceFormData.series = 'bsr'
        }
        if (val === 'ipc' && this.deviceFormData.manufacturer === 'dahua') {
          this.deviceSeriesOpts = [
            {
              value: 'HFW7233X-E2',
              label: 'HFW7233X-E2'
          ...
          this.deviceFormData.series = 'HFW7233X-E2'
        }
      } else if (val === 'nvr') {
        this.deviceFacturerOpts = this.deviceFacturerNvr
        if (val === 'nvr' && this.deviceFormData.manufacturer === 'bstar') {
          this.deviceSeriesOpts = [
            {
              value: 'Limit',
              label: 'Limit'
          ...
          this.deviceFormData.series = 'Limit'
        }
        if (val === 'nvr' && this.deviceFormData.manufacturer === 'dahua') {
          this.deviceSeriesOpts = [
            {
              value: 'DH-EVS7048S-R',
              label: 'DH-EVS7048S-R'
          ...
          this.deviceFormData.series = 'DH-EVS7048S-R'
        }
      }
    },
    'deviceFormData.manufacturer'(newval) {
      if (newval === '') {
        this.deviceFormData.manufacturer = 'bstar'
      }
      this.showFacturer(this.deviceFormData.manufacturer)
    },
  ...
  methods: {
    ...
    // 设备厂商变化
    changDeviceFacturer(val) {
      this.showFacturer(val)
    },
    showFacturer(val) {
      if (val === 'bstar') {
        if (this.deviceFormData.type === 'ipc') {
          this.deviceSeriesOpts = [
            {
              value: 'bsr',
              label: 'BSR'
          ...
          this.deviceFormData.series = 'bsr'
        } else if (this.deviceFormData.type === 'nvr') {
          this.deviceSeriesOpts = [
            {
              value: 'Limit',
              label: 'Limit'
        ...
        this.deviceFormData.series = 'onvif'
      } else if (val === 'juanxin') {
        this.deviceSeriesOpts = [
          {
            value: 'juanxin',
        ...
        this.deviceFormData.series = 'juanxin'
      } else if (val === 'dahua') {
        if (this.deviceFormData.type === 'ipc') {
          this.deviceSeriesOpts = [
            {
    ...
    deviceTableFresh() {
      ...
      this.deviceSelectIds = []
      this.deviceSlectDelIdsList = []
      this.getDevicesData(this.pageSelect)
      this.filterKey = ''
      this.isShowChild ? this.getAllDeviceCounts(-1) : this.getAllDeviceCounts(0)
    },
    // 搜索按钮获取
    ...
    deviceEditModal() {
      ...
      let index = this.deviceSelectIds[0]
      if (tab === 0 || tab === 5) {
      ...
      this.getDeviceInfo(id)
        .then(suc => {
          ...
          if (this.deviceActiveTab === 0 && !this.deviceFormData.adapterIp) {
            ...
            this.deviceFormData.adapterPort = '1' // Adapter端口
          }
      ...
      if (this.deviceActiveTab === 5) {
        this.$refs.deviceDecodeOneForm.validate(valid => {
          if (!valid) {
            ...
            this.deviceModalState.pageOneShow = true
            this.deviceModalState.pageTwoShow = false
            this.$refs.deviceDecodeTwoForm.validate()
          } else {
            this.$refs.deviceDecodeTwoForm.validate(valid => {
              if (!valid) {
                this.isDeviceValid = false
                this.deviceModalState.pageOneShow = false
                this.deviceModalState.pageTwoShow = true
