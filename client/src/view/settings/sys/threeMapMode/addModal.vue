    ...
    <Modal
        ...
        <Form class="modal-model-form" ref="formModelData" :rules="ruleValidate" :model="formModelData">
            ...
            <div class="fl">
              <FormItem label="在线" prop="uploadNameOne">
                 <Upload action="/api/setting/model/upload_gltf" style="height:34px" :show-upload-list="false" :format="['gltf']" :on-error="handleError" :on-format-error="formatError" :on-success="handleSuccessOne">
              ...
              <FormItem label="离线" prop="uploadNameTwo" v-if="nodeData.pid === '0' || nodeData.pid === '1'">
                  <Upload action="/api/setting/model/upload_gltf" style="height:34px" :show-upload-list="false"  :format="['gltf']" :on-error="handleError" :on-format-error="formatError" :on-success="handleSuccessTwo">
                  <Input :readonly='true' v-model="formModelData.uploadNameTwo" placeholder="选择Gltf格式模型文件" style="width: 250px;">
                      <Button slot="append" icon="ios-cloud-upload-outline">浏览</Button>
                  </Input>
                  </Upload>
              </FormItem>
              <FormItem label="报警" prop="uploadNameThree" v-if="!(nodeData._id === '141' || (nodeData.pid === 'root' && nodeData._id === '16')) ">
                  <Upload action="/api/setting/model/upload_gltf" style="height:34px" :show-upload-list="false"  :format="['gltf']" :on-error="handleError" :on-format-error="formatError" :on-success="handleSuccessThree">
        ...
        <div class="upload-img" v-if="!(nodeData._id === '141')">
          ...
          <div class="upload-btn">
            <Upload action="/api/setting/model/upload_pic" :show-upload-list="false" :on-error="handleError" :on-format-error="formatError" :on-success="handleSuccessPicture">
...
export default {
  data() {
    return {
      ...
      ruleValidate: { // 表单验证
        ...
        uploadNameTwo: [{ required: true, message: '请上传模型文件', trigger: 'change' }],
        uploadNameThree: [{ required: true, message: '请上传模型文件', trigger: 'change' }]
      },
      addImgSrc: ''
    }
  },
  ...
  methods: {
    ...
    modelJudge() {
      ...
      if (this.nodeData.pid === '0' || this.nodeData.pid === '1') { // 判断是否为视频和报警模型
        params.files.push({ status: 'offline', name: this.formModelData.uploadNameTwo, path: this.formModelData.pathTwo })
      }
      if (!(this.nodeData._id === '141' || (this.nodeData.pid === 'root' && this.nodeData._id === '16'))) { // 判断非巡更集合、辅助、单兵模型
        params.files.push({ status: 'alarm', name: this.formModelData.uploadNameThree, path: this.formModelData.pathThree })
      }
      if (!(this.nodeData._id === '141')) { // 判断非巡更集合
        params.picture = this.formModelData.picture
      }
      if (this.isModelAdd) { // 添加模型请求
