export default {
  ...
  data() {
    const playTimeRule = (rule, value, callback) => {
      ...
      if (r.test(value) || Number(value) === 0) {
        if (Number(value) > 9) {
          ...
          return callback(new Error('播放次数不得低于最小值1'))
        } else {
          callback()
        }
      } else {
    ...
    return {
      ...
      rankTitle: [{
        ...
        title: '地图联动图标闪烁时间(s)',
        key: 'mapIcoGlitterTime',
        width: 130,
        align: 'center'
      }, {
        title: '智能信息地图显示时间(s)',
        key: 'intelligentMsgGlitterTime',
        width: 130,
        align: 'center'
      }, {
        title: '操作',
  ...
  methods: {
    ...
    // 获取警情处理启用状态
    getAlarmDealStatus() {
      this.getAlarmDealStatusSet().then(() => {
        this.enable = JSON.parse(JSON.stringify(this.alarmDealStatus.isAlarmEnable))
      }).catch(err => {
    ...
    enableChange() {
      const payload = {
        id: this.alarmDealStatus.id,
        alarmOpen: this.enable,
        type: 'alarm'
      }
      this.reviseAlarmDealStatusSet(payload).catch(err => {
        this.enable = false
