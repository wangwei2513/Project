      ...
      <div class="advanceSelected" v-if='advancedOpen'>
        ...
        <div v-show="advanceSelected === 'yuzhiwei'" class="yuzhiwei advance-item">
          <div class="advance-box clearFloat">
            <!-- <Select v-model="presetVal"
                      size='small'
                      :disabled='presetList.length === 0||presetSetOpen'
                      multiple
                      style="width:140px;float:left; backgounrd: gray">
                <Option v-for="item in presetList"
                        :value="item.value"
            ...
            <div class="advance-btn " :class="{'dis-btn': presetList.length === 0||presetSetOpen}" @click="transPresset">调用</div>
        ...
        <bs-scroll v-show="advanceSelected === 'xunhang'" class="xunhang advance-item" :height="xunhangHeight" :class="{cruiseSetOff: !cruiseSetOpen}">
          <div class="advance-box clearFloat">
            ...
            <div class="advance-icon" v-if="showMore" @click='(cruiseSetOpen?xunhangHeight="25px":xunhangHeight="138px");cruiseSetOpen=!cruiseSetOpen'>
...
export default {
  ...
  methods: {
    ...
    transPresset() {
      ...
      this.controlDome('gotoPreset', this.speed, this.presetVal.value)
        .then(res => {
    ...
    addPreset() {
      ...
      this.controlDome('setPreset', this.speed, this.presetAllVal.label)
        .then(res => {
          if (res.status === 200) {
    ...
    async controlDome(ctrlCmdEnum, speed = this.speed, presetIndex = 1, route = 1, stopTime = 5, opt = 1, channel = 1) {
      ...
      if (!this.power) { this.power = await this.getCameraPower(item.id) }
