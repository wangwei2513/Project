<template>
  <div class="video-bottom video-buttons">
    <div class="video-bottom-left">
      ...
      <div class="realbtn iconfont" :class="[state.isPlay? '': 'disable',isPoll ? 'disable' : '', !state.isSpeech ? 'icon-shipinleiduijiangjinyong' : 'icon-shipinlei-duijiang']" title="对讲" @mouseenter="showIntercom = true" @mouseleave="showIntercom = false">
        <ul v-show="showIntercom&&state.isPlay&&!isPoll" class="intercom" style="width:140px;">
          <i class="triangle"></i>
          <li v-for="(item, i) in intercomList" @click="intercomClick(item)" :class="{active: isEnter, disable: item.display}" :key="i">{{item.label}}</li>
        </ul>
      </div>
      <div class="realbtn iconfont icon-focus" :class="{'disable' :!state.isPlay}" @click="openFocusState" title="重点关注"></div>
    ...
    <!--重点关注  -->
    <Modal v-model="openFocus" title="重点关注信息" :width="520" class="focus" :closable='false' :mask-closable="false">
      <iframe></iframe>
      <div style="position:relative">
        ...
        <div class="item">
          <table>关注类别</table>
          <VSelect v-model="atteObj.attentionType" :options="eventTypes" :maxHeight=160 id="selectType" style="width: 280px;height:32px;float:left"></VSelect>
        </div>
        <div class="item">
          <table>&nbsp;</table>
          <VSelect class="select-from" v-model="atteObj.addFollowType" :valival="getEye" :options="eventTypes" placeholder="新增关注类型" :clearSearchOnSelect="false" :isStream="true" :integarte="true" :maxHeight=160 :editable="true" id="selectType1" style="width: 280px;float:left" @search:blur="vselectBlur"></VSelect>
        ...
        <div class="item" style="height: 100px;">
          <table>&nbsp;</table>
          <Input v-model="atteObj.attentionContent" placeholder="关注内容描述" type="textarea" style="width:280px; float:left;" class='editInput' />
        </div>
        ...
        <div class="setting" v-show="openFocusSet">
          <Form ref="setting" :model="atteObj.setting" :rules="ruleValidate" :label-width="100" label-position="left">
            <Form-item label="录像时长" prop="recordTime">
              <Input v-model="atteObj.setting.recordTime" disabled placeholder="请输入" style="width:280px" />
            </Form-item>
...
export default {
  ...
  computed: {
    ...
    videotapeList() {
      return [{ value: '本地录像', label: this.localPlayer }, { value: '中心录像', label: this.centrePlayer }]
    },
    intercomList() {
      return [
  ...
  methods: {
    ...mapActions(['addFavorites', 'setFavorites', 'recordLog']),
    ...
    openRecording(val) {
      ...
      if (val.value === '本地录像') {
        ...
        if (!this.state.isRecording) {
          ...
          this.plugin.openRecording(param)
        }
        if (this.state.isRecording) {
          this.plugin.closeRecording()
    ...
    focusSave() {
      ...
      const obj = {
        ...
        attentionType: this.atteObj.attentionType.value,
