  ...
  <div v-resize="scorll" style="height:100%;overflow:hidden; padding:0 10px;">
              ...
              <bs-scroll>
                <div v-for="(v, index) in monitors" :key="v._id" style="padding: 6px 10px 6px 6px">
                  <div :class="{selected: selectedIndex == index}" @click="selectedIndex = index">
                    <Checkbox :class="'list-it'" style="width:170px;" :disabled="v.disabled&&!details.monitorChs[index]" v-model="details.monitorChs[index]">{{getMonitorName(v)}}</Checkbox>
                  </div>
                </div>
              </bs-scroll>
            </div>
    ...
    <Modal v-model="showModal" title="添加镜头" :width="622" :mask-closable="false">
      <bs-cover v-model="showModal">
        <Transfer v-if="showModal" :groupListData="details.channelNodes" @changeData="emitChangeData"></Transfer>
      </bs-cover>
      <div slot="footer" style="position:relative;z-index:99">
        <Button type="ghost" @click="(resources=null)||(showModal=false)">取消</Button>
        <Button type="primary" @click="(details.channelNodes=resources)&&(showModal=false)">确定</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    monitors() {
      ...
      const infos = this.rtscene && (this.rtscene.info || []).filter(item => item)
  ...
  methods: {
    ...
    async del(index) {
      ...
      const id = this.list[index]._id
      if (this.$lodash.find(this.rtscene.polling, item => item === id)) {
        this.showError('轮巡正在执行，无法删除')
        return
      }
      this.commonAPIHandle(this.deletePolling(this.list[index]._id), '删除', 'deletePolling')
    },
    emitChangeData(data) {
