  ...
  <div style="height: 100%;position:relative;">
    <div class="theme-bg">
      <div class="tvwall-title">
        <span>当前场景： {{curSceneName}}</span>
        <span class="title-item">当前预案： {{curPlanName}}</span>
        <span class="title-item">当前布局： {{curLayoutName}}</span>
        <span class="title-item">当前轮询： {{curpollingName}}</span>
      </div>
    </div>
    <div class="video-con">
      ...
      <div class="tv" v-else v-for="(tv, index) in tvs"
        ...
        <div class="theme-pane" v-if="!enableController" style="position:relative;height:30px;line-height:30px;padding-left: 10px;width:100%;">
          ...
          <Checkbox v-if="!tv.nodef" v-show="!isManual" style="position:absolute; right: 5px" :value="tv.type === 2" @on-change="v => changeAlarm(v, index)">设为报警屏</Checkbox>
        </div>
        <!-- </VideoFrame> -->
      </div>
...
export default {
  ...
  computed: {
    ...
    curpollingName() {
      if (this.rtscene.polling.length > 0) {
