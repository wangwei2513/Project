      ...
      <div class="form">
        <div style="width: 500px;">
          <Form ref="formValidate" :model="formValidate" :rules="validateRule" :label-width="110">
            ...
            <Form-item label="电视墙位置" :width="100" class="notRequire">
              <Input v-model="tvPosition"></Input>
            </Form-item>
...
export default {
  ...
  watch: {
    isChecked(val) {
      this.SET_ENABLE_CONTROLLER(val)
    },
    controllerInfo: {
      handler: function() {
    ...
    IPchange(item) {
      ...
      for (const i in this.IPList) {
        if (item.value === this.IPList[i].value) {
          // this.screenNum = this.IPList[i].num
