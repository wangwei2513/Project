<template>
  <div class="tv-box" style="flex:1">
    <div class="one-wall-tab">
      <TVWallLeft @changeToAuto="changeToAuto" :layoutControl="isLayout" :enableController="enableController"></TVWallLeft>
      <div class="right-con">
        <div id="btns">
          <Button-group size="small" style="border:1px solid #287fe0;border-radius: 4px;color: rgba(255, 255, 255, 0.5">
            <Button v-if="!enableController && isLayout" @click="openControlConfig">拼控配置</Button>
            <Button v-if="!enableController && !isLayout" @click="collageControl">启用拼控</Button>
            <Button :class="[!isLayout ? 'active-btn': '']" @click="switchLayout(false)">应用模式</Button>
            <Button :class="[isLayout ? 'active-btn': '']" @click="switchLayout(true)">编辑模式</Button>
          </Button-group>
        </div>
        <div>
          ...
          <TVLayout v-show="isLayout"></TVLayout>
          <TVWallApp v-show="!isLayout" ref="tvwallapp"></TVWallApp>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['SET_ENABLE_CONTROLLER', 'SET_ISLAYOUT']),
    changeToAuto() {
      this.SET_ISLAYOUT(false)
      this.$refs.tvwallapp.isManual = true
    },
    switchLayout(isLayout) {
      this.SET_ISLAYOUT(isLayout)
  ...
  created() {
    if (this.rtscene.info.length > 0) {
      this.SET_ISLAYOUT(false)
    } else {
      this.SET_ISLAYOUT(true)
    }
