...
import { AV_NVRRECORD_OPEN } from '../../../../http/video.api.js'
...
export default {
  ...
  methods: {
    ...mapActions(['recordDump', 'backupDownloadList', 'queryNVRRecordList', 'queryRecordlist', 'getPlatformID', 'gbQueryRecordList', 'gbRecordOpen', 'setResource', 'recordLog', 'getCameraPower']),
    ...mapMutations([
      'ADD_DOWNLOADLIST',
      'UPDATE_PRE',
      'SET_RESOURCE',
    ...
    async singleSearch(node, streamType, index) {
      ...
      return new Promise(resolve => {
        this.IPCPageQuery(param)
          .then(res => {
            res = JSON.parse(JSON.stringify(res))
            if (res === 'nodate') { resolve('nodata') }
            if (!res.data.result.eventList[0]) { resolve('nodata') }
            if (res === 'fail') { resolve('fail') }
            const item = res.data.result
            item.nodeName = node.name
            item.node = node
            this.SET_RESOURCE({
    ...
    singleSearchNVR(node, streamType, index) {
      ...
      return new Promise(resolve => {
        this.NVRPageQuery(param)
          .then(r => {
            if (r === 'nodate') { resolve('nodata') }
            if (r === 'fail') { resolve('fail') }
            this.SET_RESOURCE({
              index,
              item: {
                ...r,
    ...
    async singleSearchGb(node, streamType, index) {
      ...
      return new Promise(resolve => {
        this.gbQueryRecordList(param)
          .then(res => {
            if (res.data.result === 'error') { resolve('fail') }
            if (res.data.total === 0 || res.data.recordList.length === 0) { resolve('nodata') }
            this.setResource({
              index,
              item: {
    ...
    async downloadDev(record, time) {
      ...
      const format = this.parameters.videotape === 'BSR' ? '.bsr' : '.avi'
      let type = record.type
      const fileName = this.path + '\\' + record.name + '_' + time + format
      let param, openParam
      if (this.sourceType === 2) { // 前端录像
        ...
        if (record.node && record.node.nodeId) { // 国标录像
          const info = record.event
