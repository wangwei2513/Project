<template>
  ...
  <div id="play-back" class="video-pb">
    <div class="left">
      <div class="leftTop">
        <VideoMenu :synchro="synchro" ref="videoMenu" parent="playback" @changeNodeId="changeNodeId"></VideoMenu>
      </div>
      <div class="synchro">
    ...
    <div class="center">
      ...
      <div class="timeaxis">
        <TimeLine v-model="timelineValue" :disabled="disabledTimeline" :recordInfo="recordInfo" :showThumb="showThumb" :stepLength.sync="stepLength" :unParsedValue.sync="timelineValueUnParsed" @ondrag="dragTimeline" @changeSync="changeSync" @mouseDown="clearTimer" @thumbClick='thumbClick' @upTimeList='upTimeList' ref="timeLine"></TimeLine>
      </div>
    </div>

    <div class="right">
      <Retrieval tabVal="playBack" :loading="btnLoading" @on-query="queryRecord" @playNVR="playNVR" @queryNVR="queryNVR" ref="retrieval">
...
export default {
  ...
  mixins: [hook, syncPlay, asyncPlay, nvrPlay, nvrPlaySync, timelineCalc, gbPlay],
  data() {
    return {
  ...
  methods: {
    ...
    checkNextPlayingTime(time, index) {
      ...
      if (this.resourceList[index].eventList) {
        const info = this.findStrmInfo(infos, time)
        if (info) {
          return time
        }
      }
