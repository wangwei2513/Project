    ...
    <video-frame ref="frame" pluginType="record" :pluginCount="16" :clickInit="false" :defaultShowscreen="1" @update:state="setState" :slotHeight="56" @playEnd="finishPlay" @setShowscreen="num => $emit('screen', selectedShowscreen = num)" @activeChange="avtiveChange" @changePluginData="changePluginData" :bgColor="'#404040'" :fullScreenflag="fullScreenflag" :showBtn="false">
      <div class="playback">
        ...
        <div style="display: inline-block" class="leftIcon">
          ...
          <div class="dp-con" @mouseenter.stop="(hoverSpeed=selectedSpeed)&&(showSpeed=true)" @mouseleave="(hoverSpeed=selectedSpeed)&&(showSpeed=false)">
            <i class="icon iconfont" :class="[!_isPlay?'disable':'',speedIcon]" title="播放速度" style="font-size:23px;"></i>
            <ul v-show="showSpeed&&_isPlay" class="iconspeed">
        ...
        <div style="float:right">
          ...
          <div class="dp-con" @mouseenter="showTowall=true" @mouseleave="showTowall=false">
            ...
            <div v-show="showTowall&&_isPlay&&isasyn" class="showTowall">
              <iframe v-if="showTowall&&_isPlay&&isasyn"></iframe>
              <QuickToWall class="quickToWall" @checkWall="checkWall" :isPlayback="true" ref="toWall" :camera="id"></QuickToWall>
            </div>
          </div>
...
import { AV_RECORD_SIGN, AV_NVRRECORD_OPEN } from '../../../../http/video.api'
export default {
  components: {
    Slider,
    QuickToWall
  ...
  data() {
    return {
      ...
      showSpeed: false,
      speedList: [
        { value: { nRate: 1, nScale: 16 }, label: '1/16', icon: 'icon-1-16' },
        { value: { nRate: 1, nScale: 8 }, label: '1/8', icon: 'icon-1-8' },
        { value: { nRate: 1, nScale: 4 }, label: '1/4', icon: 'icon-1-4' },
        { value: { nRate: 1, nScale: 2 }, label: '1/2', icon: 'icon-1-2' },
        { value: { nRate: 1, nScale: 1 }, label: '1', icon: 'icon-speed' },
        { value: { nRate: 2, nScale: 1 }, label: '2', icon: 'icon-2' },
        { value: { nRate: 4, nScale: 1 }, label: '4', icon: 'icon-4' },
        { value: { nRate: 8, nScale: 1 }, label: '8', icon: 'icon-8' },
        { value: { nRate: 16, nScale: 1 }, label: '16', icon: 'icon-16' }
      ],
      selectedSpeed: '1',
  ...
  computed: {
    ...
    startTimeFormatted() {
      if (typeof this.startTime === 'string') {
        const date = new Date(this.formatDate.getTime())
        const st = this.startTime || '0:0:0'
        const arr = st.split(':')
        date.setHours(arr[0])
  ...
  methods: {
    ...
    openDownload() {
      ...
      const queryParam = this.resourceList[this.plugin.activedIndex].queryParam
      let timeValue = queryParam.endTime || queryParam.eTime
      timeValue = this.$parent.calendarTime
      // const stime = new Date(timeValue * 1000 - 1000)
      const stime = timeValue
      stime.setHours(0)
      stime.setMinutes(0)
      stime.setSeconds(0)
      this.startTime = stime
      // const etime = new Date(timeValue * 1000 - 1000)
      const etime = timeValue
      this.endTime = etime
      this.formatDate = timeValue
    ...
    downloadSave() {
      if (!this.startTime || !this.endTime || this.startTimeFormatted.getTime() >= this.endTimeFormatted.getTime()) {
    ...
    async dw(event, path, paramed) {
      if (event && path) {
        ...
        if (this.isNVR) {
          ...
          if (paramed.queryParam.childId) { // 下联设备
            startTime = parseInt(this.startTimeFormatted.getTime() / 1000)
            endTime = parseInt(this.endTimeFormatted.getTime() / 1000)
            ...
            //   copyList
            // )
            obj.eventList.timeInfo.startTime = startTime = parseInt(this.startTimeFormatted.getTime() / 1000)
            obj.eventList.timeInfo.endTime = endTime = parseInt(this.endTimeFormatted.getTime() / 1000)
