    ...
    <div style="display:flex;">
      <div class="modal-left">
        <Form ref="baseInformation" :model="getUserList" :rules="ruleValidate" :label-width="80" label-position="left">
          ...
          <Form-item label="NFC卡号" prop="nfc">
      ...
      <div class="modal-right">
        <div style="width:140px;height:140px;margin: 0 auto;margin-bottom:10px;border:1px dotted #fff">
          <img v-show="getUserList.photo" :src="getUserList.photo ? ('/api/upload?id='+getUserList.photo) : ''" style="width:100%;height:100%" />
        </div>
        <div style="margin: 0 auto">
          <Upload ref="upload" action="/api/upload" name="file" :max-size="20480" :format="['jpg','png','bmp','jpeg']" :on-exceeded-size="exceededSize" :on-success="uploadLaneSuc" :on-format-error="laneFormatError" :show-upload-list="false" style="text-align:center">
...
export default {
  ...
  methods: {
    ...
    uploadLaneSuc(response) {
      this.getUserList.photo = response.id
    },
