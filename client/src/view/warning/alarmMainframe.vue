    ...
    <div class="alarm-main">
      <div class="main-top">
        <div class="feature-btn mainTop-btn">
          <Select style="width:120px" v-model="onlineStatus" @on-change="searchHandle">
            <Option v-for="item in onlineStatusList" :value="item.value" :key="item.value">{{ item.label }}</Option>
          </Select>
          <Select style="width:120px" v-model="armStatus" @on-change="searchHandle">
        ...
        <div class="car-list">
          <div class="table-box">
            <Table size="small" height="300" :columns="alarmTitle" :data="alarmData" :highlight-row="true" @on-row-click="clickDeviceRow" @on-selection-change="selectAlarmHost" :row-class-name="rowClass"></Table>
          </div>
        </div>
      ...
      <div class="main-bottom" :style="{height: tableBoxHeight}" ref="tableBox">
        <div class="mainBtm-left">
          <div class="feature-btn">
            ...
            <Button type="ghost" @click="resHandle('remove')">清除</Button>
...
export default {
  ...
  data() {
    return {
      ...
      allAlarmStatusList: [], // 所有报警主机及防区的状态列表
      timerId: null,
      isWait: false,
  ...
  methods: {
    ...
    getAllAlarmStatus() {
      ...
      this.getAllAlarmDevStatus({ devIp: 'shike', devPort: 2302 }).then(res => {
        this.isWait = false
        this.allAlarmStatusList = res.data || []
        this.alarmData.forEach(item => {
          for (let i = 0; i < this.allAlarmStatusList.length; i++) {
            if (item.ip === this.allAlarmStatusList[i].devIp) {
              this.$set(item, 'alarmStatus', this.allAlarmStatusList[i].armStatus)
              break
            }
          }
        })
    ...
    getAllAlarmStatusAuto() {
      this.getAllAlarmDevStatusAuto({ devIp: 'shike', devPort: 2302 }).then(res => {
        this.allAlarmStatusList = res.data || []
        this.alarmData.forEach(item => {
          for (let i = 0; i < this.allAlarmStatusList.length; i++) {
            if (item.ip === this.allAlarmStatusList[i].devIp) {
    ...
    getResData() {
      this.getResOfDeviceByMainId({ eid: this.deviceRowId })
        .then(res => {
          this.alarmInData = this.alarmInDataById
          this.alarmInRowId = ''
          let channelStatus = []
          this.getAlarmHostPower(this.alarmInData)
          for (let i = 0; i < this.allAlarmStatusList.length; i++) {
            if (this.alarmInData[0].ip === this.allAlarmStatusList[i].devIp) {
              channelStatus = this.allAlarmStatusList[i].channelStatus
              break
            }
          }
    ...
    devHendle(methType) {
      ...
      switch (methType) {
        case 'protection':
          ...
          if (!hasPower) { return }
          this.protectionAction({ ids: this.selectHostIdList, type: 'dev' })
            .then(() => {
              const param = {
                logType: '操作日志',
                module: '报警处理',
        ...
        case 'removal':
          ...
          if (!hasPower) { return }
          this.removalAction({ ids: this.selectHostIdList, type: 'dev' })
            .then(() => {
              const param = {
                logType: '操作日志',
                module: '报警处理',
        ...
        case 'remove':
          ...
          if (!hasPower) { return }
          this.removeAction({ ids: this.selectHostIdList, type: 'dev' })
            .then(() => {
              const param = {
                logType: '操作日志',
                module: '报警处理',
    ...
    resHandle(methType) {
      ...
      switch (methType) {
        case 'protection':
          ...
          this.protectionAction({ ids: this.selectSectorIdList, type: 'res' })
            .then(() => {
              const param = {
                logType: '操作日志',
                module: '报警处理',
        ...
        case 'removal':
          ...
          if (!sectorPower) { return }
          this.removalAction({ ids: this.selectSectorIdList, type: 'res' })
            .then(() => {
              const param = {
                logType: '操作日志',
                module: '报警处理',
        ...
        case 'remove':
          ...
          if (!sectorPower) { return }
          this.removeAction({ ids: this.selectSectorIdList, type: 'res' })
            .then(() => {
              const param = {
                logType: '操作日志',
                module: '报警处理',
        ...
        case 'branch':
          ...
          if (!sectorPower) { return }
          this.branchAction(this.selectSectorIdList)
            .then(() => {
              const param = {
                logType: '操作日志',
                module: '报警处理',
        ...
        case 'withdraw':
          ...
          if (!sectorPower) { return }
          this.withdrawAction(this.selectSectorIdList)
            .then(() => {
              const param = {
                logType: '操作日志',
                module: '报警处理',
