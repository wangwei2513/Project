...
const actions = {
  ...
  // 根据地图范围统计资源
  getStaticDataByMapExtent({ commit }, playod) {
    return new Promise((resolve, reject) => {
      getStaticDataByMapExtentApi(playod)
        .then(res => {
          commit('SET_SELECT_BOX_VIDEO_DATA', res.data)
          resolve()
        })
