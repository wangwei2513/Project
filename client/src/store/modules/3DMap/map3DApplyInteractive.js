...
export default {
  ...
  getters: {
    // 点选状态
    isPointChooseStatus: state => {
      return state.isPointChooseStatus
    },
    // 框选二级面板显隐状态
    ...
    isOnlyAlarm(state) {
      // 报警点位-仅报警时显示
      return state.pointSH.isOnlyAlarm
    },
    isAlarm(state) {
      // 点位-普通报警
  ...
  mutations: {
    ISPOINTCHOOSESTATUS(state, v) {
      state.isPointChooseStatus = v
    },
    BOX_CHOOSE_SEC_PANEL_STATUS(state, v) {
      state.boxChooseSecPanelStatus = v
    },
    CHANGE_IS_REC_ALARM(state, v) {
      state.isRecAlarm = v
    },
    CHANGE_IS_EMERGENCY(state, v) {
      state.isEmergency = v
    },
    CHANGE_POINT_SH(state, { type, val }) {
      state.pointSH[type] = val
    },
  ...
  actions: {
    // ISPOINTCHOOSESTATUS
    changePointChooseStatus({ commit }, v) {
      commit('ISPOINTCHOOSESTATUS', v)
    },
  ...
  switch (params.type) {
    case 0: // 视频
      return {
        ...
        contactsInfo: params.data.point3D
          ? params.data.point3D.principal.map(v => {
            return {
              contacts: v.name || '',
