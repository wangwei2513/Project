...
const mutations = {
  ...
  SET_ORIGIN_LIST(state, list) {
    ...
    list.forEach(item => {
      ...
      obj.num = Number(obj.origin.slice(-1))
      if (arr.length === 0) {
  ...
  SAVE_ERROR_INFO(state, info) {
    if (info === false) {
      return (state.errorInfoArray = [])
    }
    state.errorInfoArray.push(info)
  }
}
const actions = {
  saveErrorInfo({ commit }, info) {
    commit('SAVE_ERROR_INFO', info)
  },
  getTVList({ commit, state }) {
    return new Promise((resolve, reject) => {
      getTVListApi()
        .then(resp => {
          // 提交错误信息
          // commit('SAVE_ERROR_INFO', resp)
