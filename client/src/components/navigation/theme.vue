<template>
  <div class="navigation"
    ...
    <div v-if="loggedIn">
      <div class="theme-container"
        ...
        <div class="header-menu">
          <ul class="theme-box">
            <!-- <li class="theme-item theme-item-first" @click="skipTo('/navigation')">导航</li> -->
            <li v-if="!($route.path==='/navigation')"
              class="theme-item theme-item-first"
              @click="skipTo('/navigation')">导航</li>
            <!-- <li v-for="munuitem in momentmunuList.children" class="theme-item theme-item-first" :class="{active:momentTitle === munuitem.url}" @click="skipTo(munuitem.url)" :key="munuitem.tag">{{munuitem.name}}</li> -->
            <li v-if="($route.path !== '/navigation') && isMapNavShow(munuitem)" v-for="munuitem in momentmunuList.children" class="theme-item theme-item-first" :class="{active:momentTitle === munuitem.url}" @click="skipTo(munuitem.url)" :key="munuitem.tag">{{munuitem.name}}</li>
          </ul>
        </div>
        ...
        <div v-if="pullDownFlag"
          class="pull-down"
          :style="{fontStyle:fontItalic,color:fontColor,fontWeight:fontRegular,textDecoration:fontUnderline,fontFamily:fontfamily,backgroundColor:uploadColor, height:duty==='yes' ? '125px' : '83px'}">
          <iframe></iframe>
          <div class="modal-mask"
            @click='backPopTime'></div>
          <div class="list-box"
        ...
        <div v-if="showAlarm"
          ...
          <div class="list-box"
            ...
            @click="jumpAlarmUrl">
            <p class='showInfor' :title="allAlarmNewOneData.name">报警名称：{{allAlarmNewOneData.name}}</p>
    ...
    <div v-show="$route.fullPath === '/map/3D'"
      class="btn-shrink"
...
export default {
  ...
  computed: {
    ...
    mapType() {
      return '/map/' + read('mapType')
    },
    mapEditType() {
      return '/mapEdit/' + read('mapType')
    }
  },
  watch: {
    getMenuStyle: {
    ...
    allAlarmNewOneData(newVal) {
      if (
        ...
        (window.location.href === 'http://' + window.location.host + '/warning/receive' && (this.allAlarmNewOneData.eventType !== 'fireAlarm' && this.allAlarmNewOneData.eventType !== 'fireFailure'))
      ) {
  ...
  methods: {
    ...
    isMapNavShow(munuitem) {
      return (
        munuitem.url === this.mapType ||
        munuitem.url === this.mapEditType ||
