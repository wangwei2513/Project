  ...
  <div class="org-actions-box" :class="{orgHeight: !isShowTitle}">
    ...
    <div class="box-btn" v-if="isSetting && isHandle">
      <Button-group>
        ...
        <!-- <Button @click="openMoveModal('left')" title="升级" :disabled="isHighOrg || isRootOrg"><i class="icon iconfont icon-upper-level"></i></Button> -->
        <!-- <Button @click="openMoveModal('right')" title="降级" :disabled="isRootOrg || isTopOrg"><i class="icon iconfont icon-nextlower"></i></Button> -->
        ...
        .then(suc => {
          ...
          if (orgInfo.isroot) {
            if (way === 'add') {
              ...
              // let codeRandom = parseInt(Math.random() * 655)
              this.isNew = true
              this.orgFormData = {
                code: '',
                pid: this.activeId,
