  ...
  <div id="alarmVideoMadel">
    ...
    <div class='videoPart'>
      <div class='operaVideo'>
        <span class="item iconfont icon-shipinlei-duijiang" title="对讲"></span>
    ...
    <Timeline v-model="timelineValue" :disabled="false" :recordInfo="recordInfo" @forwardTop="handleDragTop"  @ondrag="handleDrag"  @mouseDown="clearTimer" ref="timeline" :isModel="true" style="width: calc(100% - 15px);"></Timeline>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    openPlayback(index, param) {
      this.getPlugin(index).videoPlay(param)
    },
    getTimeValue(type) {
      if (type !== 'preview') {
