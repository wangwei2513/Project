...
<template>
  <div class="MapVideoPlayback" style="height: 100%; padding:6px 5px; 0">
    ...
    <div style="display:inline-block; vertical-align:middle;height:100%;width: 550px;">
      ...
      <div class="btn-con" style="display:inline-block;margin-left:10px; width: 315px;overflow:hidden">
      ...
      <div :class="playBackAllButton ? 'btnConDisabled' :'btn-con'" style="float:right;margin-right:10px;line-height:42px;" class="iconfont">
        ...
        <div class="setVolume" @mouseenter="showVolume = true" @mouseleave="showVolume = false">
          ...
          <!--  -->
          <div class="slider-box" style="" :style="{bottom: (showVolume || isDragging)&&pluginState.isVolumeOpen? '-16px': '-9999px' }">
...
export default {
  ...
  methods: {
    ...
    async searchRecord() {
      ...
      if ((!this.power || !this.power.includes('playbackDownload')) && !this.videoParam.type) {
    ...
    dw(path) {
      ...
      if (dump.success) {
        this.$Notice.warning({ title: '提示', desc: '录像开始下载' })
