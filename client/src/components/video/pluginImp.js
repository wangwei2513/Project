...
//   extend
// } from 'jquery'
import { AV_STREAM_START, AV_RECORD_LIST, AV_NVRRECORD_OPEN, getDownID } from 'http/video.api'
...
const common = {
  ...
  getCapture(plugin, param) {
    return plugin.GetRealPicturebyBase64(param.type, param.quality)
  },
  openSound(plugin) {
...
const recordPlugin = {
    ...
    return new Promise(async(resolve, reject) => {
      ...
      try {
        ...
        const pluginParam = JSON.stringify({
          ...
          session: '11',
          ip: obj.queryParam.devIp,
          channel: obj.queryParam.channel,
          port: obj.queryParam.devPort + '',
  ...
  recordSave(plugin, { startTime, endTime, strmInfo, dsIp, path }) {
    const obj = {
      eventList: {
        timeInfo: {
          startTime: startTime,
