<template>
  <div class="container" :class="{ active: isActive&&!noBorder, bdr: boder, bdr1: !boder}" @click="clickEvent" @dblclick="doubleClickEvent" :index="index" @mouseout="startShowTimer">
    <div style="postion:relative;height:100%;vertical-align:middle" :class="{outofscreen: isSyncPause || isCapPause}" :index="index">
      <div v-if="init" class="objectBox">
        <object ref="object" type="application/x-webplayercontrol" ></object>
      </div>
    </div>
    <img style="postion:relative;height:100%;vertical-align:middle;width:100%" v-if="isCapPause" :src="capImg" />
    <div style="postion:relative;height:100%;vertical-align:middle;background:black" v-if="isSyncPause"></div>
    <!--
    ...
    <div v-if="isReviewOpen" class="buttonBar" @mouseout.stop="startShowTimer" @mouseover.stop>
      ...
      <div class="buttonBar-box">
        ...
        <div style="float: right;">
          ...
          <!-- 进度条 -->
          <playbackProgress :allTime="BACKTIME" :basics="VnowTime" :value="backPlan" :disabled="false" style="width: 240px;height:20px; margin-left:10px; display:inline-block" @on-change="backMousehange" @on-mousedown="backMousedown" @on-mouseup="backMouseup"></playbackProgress>
...
export default {
  ...
  methods: {
    ...mapActions(['gbRecordOpen', 'getCameraPower', 'setVerifacePlayingCameraList', 'upPlayindId']),
    removePlugin() {
      // ff bug: 不删除插件
    ...
    async open(param, noPower) {
      ...
      if (this.pluginType === 'preview' && this.pluginState.isPlay && param.gbDevId) { // 国标切换码流需要先关流
    ...
    quickStop() {
      ...
      if (this.$route.path === '/veriface/Capture') {
        this.setVerifacePlayingCameraList({ index: this.$parent.activedIndex, data: [] })
      }
    ...
    async NVRopen(data) {
      ...
      const state = await this.pluginAPI.NVRopen(this.plugin, data)
      if (state.state === 0) {
        this.pluginState.isStopped = false
        this.pluginState.isPlay = true
        this.$nextTick(_ => {
    ...
    async gbOpen(data) {
      ...
      const state = await this.pluginAPI.gbOpen(this.plugin, data, res)
      if (state === 0) {
        this.pluginState.isStopped = false
        this.pluginState.isPlay = true
