  ...
  <Modal v-model="modalShow" :title="'设备在线详情'" :width="ModalWidth" :mask-closable="false" :closable='false' @mousedown.stop @mouseup.stop class="Vmodel">
    ...
    <div class="modol-body" ref='modelBody'>
      ...
      <div class="modol-body-right rt" ref="childBox" :style="{width: `calc(100% - ${isDevice ? '100px' : '175px'})`}">
        <OnlineTime v-if='modalShow' ref="child" :isFullScreen='isFullscreen' :timeAllHeight='timeAllHeight' :detailData='detailData' :headerShow='true' :offlineColor='offlineColor' :basicColor='basicColor' :onlineColor='onlineColor'></OnlineTime>
      </div>
      <div v-if='timeData.length === 0' class="no-data">暂无数据</div>
    </div>
    <div slot="footer" class="footer">
      ...
      <Page class="rt" @on-change='changeCurrent' @on-page-size-change='changePageSize' :current='page' :total='counts' :show-sizer='showSizer' :page-size='pageNum' :page-size-opts='pageNumArray'></Page>
    </div>
  </Modal>
</template>
