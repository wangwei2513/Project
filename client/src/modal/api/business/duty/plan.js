 ...
 * 增加计划
 */
function create({ name, startTime, endTime, template, group }) {
  let params = {
    name,
    startTime,
    endTime,
  ...
  return $http
    .post('/business/duty/plan', params)
    .then(res => res.data)
    .catch(err => {
      return Promise.reject(err.response.data)
    })
 ...
 * 修改计划
 */
function modify({ id, name, startTime, endTime, template, group, detail }) {
  let params = {
    _id: id,
    name,
    startTime,
  ...
  return $http
    .put(`/business/duty/plan/${id}`, params)
    .then(res => res.data)
    .catch(err => {
      return Promise.reject(err.response.data)
    })
