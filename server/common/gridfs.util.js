 ...
 * @Last Modified time: 2018-09-19 10:52:55
 */
'use strict'
...
// const DB_CONN_STR = 'mongodb://192.168.20.5/test'
...
const getWriteBucketName = async (type) => {
  if (!global.createFileConnectTime) { // 服务器重启全局变量失效
...
const getReadBucketName = async (db, id) => {
  ...
  connections.forEach(n => {
    if (n.collectionName.endsWith('.files')) {
      ...
      if (nameStr) { connectNames.push(nameStr[0]) }
    }
  })
  return prefix + connectNames.sort().reverse().find(n => n < timestamp)
}
