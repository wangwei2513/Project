 ...
 * @Last Modified time: 2018-11-14 14:56:07
 */
...
exports.getTree = async ctx => {
  try {
    ...
    // 获取所有楼层
    let buildings = await Building.find({ gid: { $exists: false }, mapId: ctx.query.mapId }, '_id code name').lean().exec()
    ...
    // 获取楼宇
    let stroeys = await Storey.find({ bid: { $in: bids } }).lean().exec()
...
exports.statistic = async ctx => {
  try {
    ...
    const resources = await Resource.find({ point3D: { $nin: [null, undefined] } }).lean().exec()
    const buildings = await Building.find({}, 'loc center').lean().exec()
    let online = 0
    let offline = 0
    let buildLength = 0
    for (var item of resources) {
      if (_.isEmpty(item.point3D)) { continue }
...
exports.statisticResource = async ctx => {
  try {
    ...
    const resources = await Resource.find(query).lean().exec()
    const reses = []
    for (var item of resources) {
      if (_.isEmpty(item.point3D)) { continue }
      const pointArr = item.point3D.loc.split(',')
...
exports.getAllResource = async ctx => {
  try {
    ...
    const [buildings, ipcs, fires, patrols] = await Promise.all([
      Building.find({}).skip((+buildingSkip) * (+limit)).limit(+limit).exec(),
      Resource.find({ point3D: { $nin: [null, undefined] }, type: { $in: [0, 1] } }, 'chan name monitortype status eid alarmintype alarmtemplate maxdelaytime minintervaltime mapsign alarmaffirm point3D').skip((+ipcSkip) * (+limit)).limit(+limit).exec(),
      Resource.find({ point3D: { $nin: [null, undefined] }, type: { $in: 11 } }, 'chan name monitortype status eid alarmintype alarmtemplate maxdelaytime minintervaltime mapsign alarmaffirm point3D').skip((+fireSkip) * (+limit)).limit(+limit).exec(),
      PatrolPoint.find({ point3D: { $nin: [null, undefined] } }).skip((+patrolSkip) * (+limit)).limit(+limit).exec()
    ])
    ctx.body = {
      buildings,
      ipcs,
