...
exports.batchUpdate = async (ctx, next) => {
  ...
  try {
    ...
    if (!_.isEmpty(data.newData) || !_.isEmpty(data.varyData)) {
      notice.info(ctx, { module: 'recordcfg', data }).catch(err => {
...
const findDiff = async (data, Model) => {
  ...
  // 为新建数据查找对应的资源并生成发送格式
  if (!_.isEmpty(fresh)) {
    const res = await Resource.find({ _id: { $in: _.map(fresh, 'resource') } }).populate('eid').lean()
    for (const item of fresh) {
      const target = _.find(res, doc => doc._id.toString() === item.resource)
      collection.newData.push({ devIp: target.eid.ip, channel: target.chan, dsServerId: item.server, devPort: target.eid.cport, path: item.path })
    }
  }
  // 如果没有已经在数据，则直接返回
  if (_.isEmpty(keep)) { return collection }
  // 处理所有已经的数据
  const result = await Model.find({ _id: { $in: _.map(keep, '_id') } }).populate({ path: 'resource', populate: { path: 'eid' } }).lean()
  for (const item of keep) {
    const target = _.find(result, doc => doc._id.toString() === item._id)
    if (item.path === target.path && item.server === target.server) { continue }
    collection.varyData.push({
      before: { devIp: target.resource.eid.ip, channel: target.resource.chan, dsServerId: target.server, devPort: target.resource.eid.cport, path: target.path },
      after: { devIp: target.resource.eid.ip, channel: target.resource.chan, dsServerId: item.server, devPort: target.resource.eid.cport, path: item.path }
    })
  }
  return collection
}
