...
exports.index = async (ctx, next) => {
  ...
  try {
    const result = await paging.listQuery(Record, search, fileds, {}, paganation, { path: 'planTemplateId', select: 'name' }, ctx)
    if (_.isNull(result)) {
...
const inspectDiff = async (data, Model) => {
  ...
  for (const item of keep) {
    ...
    if (item.enable !== target.enable || item.streamType !== target.streamType || item.planTemplateId !== target.planTemplateId.toString()) {
      return true
    }
  }
  return false
