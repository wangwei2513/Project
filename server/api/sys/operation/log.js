 ...
 * @Last Modified time: 2018-12-24 10:20:04
 */
let { devLog, recordLog, serverLog } = require('./model.base')
...
exports.serviceState = async ctx => {
  ...
  let OnLines = await Promise.all(
    serviceData.map(async item => {
      const logList = await serverLog
        .find({ ip: item._id.ip, serverType: item._id.serverType, createTime: { $gte: item.createTime } }, 'ip status createTime')
      let DevStatus = {
        ip: item._id.ip,
        name: item.name,
