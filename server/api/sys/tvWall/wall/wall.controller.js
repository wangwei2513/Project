 ...
 * @Last Modified time: 2018-12-20 14:44:16
 */
...
const { setMonitorCfg, getMonitorCfg, openWall, openVodWall, frontVodWall, closeWall, closeAll, keepingState, getState, switchScene, vallDisplay, getJointLayout, setJointLayout, fullDisplay, gbopen } = require('../../../bstar/tvwall.interface')
const { getNowScene } = require('../tool')
...
const getNewScene = async (ctx, rtscene) => {
  try {
    ...
    try {
      ...
      ctx.throw(500, { code: error.error ? error.error : 3, message: '请求第三方接口异常', type: 'sys' })
    }
    const newScene = {
      name: rtscene.name,
      wall: rtscene.wall,
    ...
    if (_.isEmpty(state.monitorConf)) {
      ...
      for (const item of state.monitorConf) {
        ...
        for (var temp of item.panePlay) {
          ...
          const resource = await Resource.findOne({ ip: temp.devIp, chan: parseInt(temp.channel) }).exec()
          obj.code = temp.pane || ''
          obj.resource = resource ? resource._id : ''
          obj.code && obj.resource && infoItem.resources.push(obj)
        }
        // 查找出监视器对应的解码器通道
        const monitorChan = await Resource.findOne({ ip: item.devIp, chan: item.monitor, type: 5 }, '_id').lean().exec()
        const monitor = await Monitor.findOne({ channel: monitorChan._id, wall: rtscene.wall + '' }, ' _id').lean().exec()
        if (!monitor) { continue }
        infoItem.monitor = monitor ? monitor._id.toString() : ''
        infoItem.type = sceneObj[monitor._id + ''].type || 0
...
//   }
// }
exports.get = async ctx => { // 获取电视墙的所有配置信息
  try {
    ctx.set('loginfo', encodeURI('电视墙-获取电视墙配置'))
    const walls = await Wall.find().populate('layout').exec()
    ...
    const wallsCfg = await Promise.all(walls.map(async item => {
      item = item.toObject()
...
// 获取电视墙列表
exports.getList = async ctx => { // 获取电视墙的所有配置信息
  try {
    ctx.set('loginfo', encodeURI('电视墙-获取电视墙配置'))
    const walls = await Wall.find({}).populate('layout rtscene').lean().exec()
    const origins = await Origin.find({}).lean().exec()
    const originMapping = {}
    for (var item of origins) {
      let monitor
      item.decodechan && (monitor = await Monitor.findOne({ channel: item.decodechan }, '_id').lean().exec())
  ...
  try {
    ...
    if (ctx.request.body.jointcontroller) {
      try {
        ...
        const layout = await Layout.findOne({ sceneid: -1 }, '_id wininfo').populate({ path: 'wininfo.monitor', select: 'channel startcode', populate: { path: 'channel', select: 'ip port chan' } }).lean().exec()
        wallDocs.layout = layout._id
        setMonitorReqBody.sceneswitch[0].monitorConf = layout.wininfo.filter(item => item.monitor).map(item => {
          return {
...
async function initOriginMonitorScene (ctx, wall) {
  try {
    ...
    if (_.isEmpty(origin)) {
      // 查找出已经配置监视器的解码通道
      const monitorDecodeChan = await Monitor.find({}, 'channel').lean().exec()
      const ids = monitorDecodeChan.map(item => item.channel)
      // 查找出所有的解码通道
      const [decodeChan, jointChan] = await Promise.all([
        Resource.find({ type: 5, _id: { $nin: ids } }, '_id chan eid').populate({ path: 'eid', select: 'ip cport username password' }).sort('chan').exec(),
        Resource.find({ type: 15 }, '_id chan').sort('chan').exec()
      ])
      if (!decodeChan || !decodeChan.length) { ctx.throw(500, { code: 500, message: '无可用的解码通道' }) }
      const oriArr = []
      const monitorArr = []
      ...
      while (i < jointChan.length) {
        ...
        if (decodeChan[i]) {
          oriObj.decodechan = decodeChan[i]._id
        }
        if (jointChan[i]) {
          oriObj.jointorigin = jointChan[i]._id
        }
        oriArr.push(oriObj)
        if (decodeChan[i] && jointChan[i]) {
...
async function initJointLayout (ctx, wall, devinfo) {
  try {
    ...
    try {
      ...
      ctx.throw(500, { code: error.error, message: '请求第三方接口异常', type: 'sys', options: { url: error.options.baseUrl + error.options.url, reqBody: error.options.body } })
    }
    const jointDev = await Device.findOne({ ip: devinfo.devIp, cport: devinfo.port }, '_id').lean().exec()
    const jointRes = await Resource.find({ eid: jointDev._id }, '_id chan').lean().exec()
...
exports.getMonitorByWall = async ctx => {
  try {
    ...
    const wall = await Wall.findById(ctx.params.id).populate('rtscene').lean().exec()
    const rtscene = wall.rtscene.info
    const rtsceneMapping = {}
    rtscene.forEach(item => {
      rtsceneMapping[item.monitor + ''] = item.panecount
    })
    const monitors = await Monitor.find({ wall: ctx.params.id }).populate([
      {
        path: 'equipment',
...
exports.getPollingByWall = async ctx => {
  try {
    ...
    const pollings = await Polling.find({ wall: ctx.params.id }).populate({ path: 'channels', select: 'name' }).exec()
    ctx.status = 200
    ctx.body = pollings
  } catch (error) {
    console.log(error)
...
exports.delete = async ctx => {
  try {
    ...
    const wallDoc = await Wall.findById(ctx.params.id).lean().exec()
...
exports.open = async ctx => {
  try {
    ...
    const monitorObj = await Monitor.findById(monitor).populate('equipment channel').exec()
    const resourceObj = await Resource.findById(resource).populate('eid').exec()
    const data = {
    ...
    try {
      if (resourceObj.gbDevId && resourceObj.nodeId) {
        body.devCtl = {
          ...
          gbDevId: resourceObj.gbDevId,
          gbPlaNvrId: resourceObj.gbPlaNvrId || ''
        }
        await gbopen(ctx, body)
      } else {
        body.devCtl = {
          monitor: monitorObj.channel.chan,
          streamType: monitorObj.channel.stream,
      ...
      return ctx.throw(500, { code: error.error ? error.error : 3, message: '请求第三方接口异常', type: 'sys' })
    }
    const { rtscene } = await Wall.findById(ctx.params.id).populate('rtscene').exec()
    let newScene = null
    try {
...
exports.close = async ctx => {
  try {
    ...
    const monitorObj = await Monitor.findById(monitor).populate('equipment channel').exec()
    const { rtscene } = await Wall.findById(ctx.params.id).populate({
      path: 'rtscene',
      populate: {
        path: 'polling',
        select: 'monitorsinfo'
...
exports.setMonitorCfg = async ctx => {
  try {
    ...
    rtscene.info.forEach(item => {
      if (item.monitor + '' === monitor + '') { item.panecount = panecount }
    })
    let newScene = null
    try {
      newScene = await getNewScene(ctx, rtscene)
...
exports.closeAll = async ctx => {
  try {
    ...
    const { rtscene } = await Wall.findById(wallid).populate('rtscene').exec()
    ...
    //   return ctx.throw(500, { code: 3, message: '请求第三方接口异常' })
    // }
    rtscene.info.forEach(item => {
      item.resources = []
    })
...
exports.showNo = async (ctx) => {
  try {
    ...
    const datas = await Monitor.find({ _id: { $in: monitors } }, 'code channel equipment').populate([{ path: 'equipment', select: '_id ip cport' }, { path: 'channel', select: 'chan' }]).exec()
    const _body = { decoderList: [] }
    datas.forEach(n => _body.decoderList.push({ devIp: n.equipment.ip, devPort: n.equipment.cport, display: show, code: n.code, chan: n.channel.chan }))
...
exports.getJointLayout = async (ctx) => {
  try {
    ...
    const jointDev = await Device.findOne({ ip: ctx.request.body.ip, cport: ctx.request.body.port }, '_id').lean().exec()
    const jointRes = await Resource.find({ eid: jointDev._id }, '_id chan').lean().exec()
    const jointResMapping = {}
    jointRes.forEach(res => {
      jointResMapping[res.chan] = res._id
    ...
    layout.name = data.sceenScenesInfoArr[0].scenesInfoArr[0].swScreenInfo.rHScreenNum + '*' + data.sceenScenesInfoArr[0].scenesInfoArr[0].swScreenInfo.rVScreenNum
...
exports.setJointLayout = async (ctx) => {
  try {
    ...
    // 更新当前电视墙的布局
    const layout = await Layout.findOne({ sceneid: ctx.request.body.sceneid }, '_id').lean().exec()
...
exports.getDecodeChan = async ctx => {
  try {
    const res = await Resource.find({ type: 5 }, 'ip port name').lean().exec()
    ctx.status = 200
    ctx.body = res
  } catch (error) {
    handleSysException(error)
...
exports.updateAllJointLayout = async ctx => {
  try {
    ...
    // 获取改电视墙下的所有监视器
    const monitors = await Monitor.find({ wall: ctx.request.body.wall }, '_id jointorigin').lean().exec()
    const monotorMapping = {}
    monitors.forEach(m => {
      monotorMapping[m.jointorigin + ''] = m._id + ''
    })
    ...
    // 更新布局
    await Promise.all([...updateDocs.map(item => Layout.findOneAndUpdate({ sceneid: item.sceneid }, item).exec()), Layout.create(createDocs)])
    ctx.status = 200
  } catch (error) {
    handleSysException(error)
  }
    ...
    if (showMax) {
      ...
      try {
        ...
        cfg.MonitorCfgPrmArr.forEach(item => {
          if (monitorObj.channel.chan + '' === item.monitor + '') { panecount = item.paneCnt }
        })
      } catch (error) {
        return ctx.throw(500, { code: error.error, message: '请求第三方接口异常', type: 'sys' })
      }
