 ...
 * @Last Modified time: 2018-11-19 17:25:43
 */
...
exports.modify = async ctx => {
  try {
    ...
    if (!ctx.request.body.wall) { ctx.throw(500, { code: 11, message: '参数不能为空' }) }
    const wall = await Wall.findById(ctx.request.body.wall).exec()
    if (wall.selectedplan && wall.selectedplan + '' === ctx.params.id + '') { ctx.throw(500, { code: 7, message: '正在执行，不能修改' }) }
    ...
    const body = {
      scenceList: plan.toObject().info.map(planinfo => {
        return {
          isSceneswitch: true,
          cameraGroup: {
            cameraList: !planinfo.scene.polling ? [] : planinfo.scene.polling.channels.map(channel => {
              return {
          ...
          monitorConf: planinfo.scene.info.map(item => {
            return {
              ...
              playType: item.type === 2 ? 2 : planinfo.scene.polling && planinfo.scene.polling.monitorsinfo.findIndex(monitor => monitor + '' === item.monitor._id + '') !== -1 ? 1 : 0,
...
exports.use = async ctx => {
  try {
    ...
    const { jointcontroller } = await Wall.findById(plan.wall, 'jointcontroller').lean().exec()
    console.log('plan-----', JSON.stringify(plan))
    const body = {
      scenceList: plan.info.map(planinfo => {
        let pollingMonitors = []
        planinfo.scene.polling.length && planinfo.scene.polling.forEach(item => {
