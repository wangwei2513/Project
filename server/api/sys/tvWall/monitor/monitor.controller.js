 ...
 * @Last Modified time: 2018-09-14 15:08:29
 */
...
exports.getByWall = async ctx => {
  try {
    ...
    const monitors = await Monitor.find({ wall: ctx.params.id }).populate('equipment channel').exec()
    ctx.status = 200
    ctx.body = monitors
  } catch (error) {
    console.log(error)
...
exports.add = async ctx => {
  try {
    ...
    const resource = await Resource.findById(rqbody.channel).populate('eid').exec()
    // 判断通道是否被占用
    const channels = await Monitor.find({ wall: rqbody.wall }, 'channel').exec()
    let flag = false
    channels.forEach(item => {
      if (item.channel + '' === rqbody.channel + '') { flag = true }
    })
    if (flag) { ctx.throw(500, { code: 10, message: '解码通道已被占用' }) }
    if (!resource) { return ctx.throw(500, { code: 2, message: '该资源不存在' }) }
    // 初始化窗格分割为1,
    const body = {
      devInfo: {
    ...
    if (!cfg) { return ctx.throw(500, { code: 4, message: '资源通道不存在' }) }
    rqbody.startcode = cfg.paneTag
    const monitor = new Monitor(rqbody)
    const doc = await monitor.save()
    let { rtscene } = await Wall.findById(rqbody.wall).populate('rtscene').exec()
...
exports.modify = async ctx => {
  try {
    ...
    let flag = false
    channels.forEach(item => {
      if ((item.channel + '' === rqbody.channel + '') && (item._id + '' !== ctx.params.id + '')) { flag = true }
    })
    if (flag) { ctx.throw(500, { code: 10, message: '解码通道已被占用' }) }
...
exports.delete = async ctx => {
  try {
    ...
    for (var id of ids) {
      ...
      let { rtscene } = await Wall.findById(monitorObj.wall).populate('rtscene').exec()
      rtscene = rtscene.toObject()
