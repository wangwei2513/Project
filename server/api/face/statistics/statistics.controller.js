...
const PeopleCountLog = db.model('PeopleCountLog', new mongoose.Schema({
  insidepeople: Number,
  exitpeople: Number,
  vendor: Number,
  starttime: Number,
...
const AlarmLog = db.model('AlarmLog', new mongoose.Schema({
  eventType: String,
  time: Number
}), 'AlarmLog')
...
exports.contrast = async (ctx, next) => {
  ...
  _.isUndefined(search.granularity)
    ...
    : CONSTANTS.GRANULARITY.RANGE.includes(Number(search.granularity)) ? query.granularity = Number(search.granularity) : query.granularity = _.first(CONSTANTS.GRANULARITY.RANGE)
  // 时间是否连续默认值处理
  ...
  _.isUndefined(search.type)
    ...
    : CONSTANTS.PEOPLE.UNUSUALSTATIC.includes(Number(search.type)) ? query.type = Number(search.type) : query.type = _.first(CONSTANTS.PEOPLE.UNUSUALSTATIC)
  try {
...
exports.outline = async (ctx, next) => {
  ...
  try {
    ...
    // 获取布控总数
    const attentionCout = await People.countDocuments({ type: CONSTANTS.PEOPLE.ATTENTION, status: CONSTANTS.ATTENTION.GOING })
    // 获取今天到目前布控人员发现数量
    const attentionFind = await Face.countDocuments({ snapshotTime: { $gte: Number(moment(date).format('X')) }, type: CONSTANTS.PEOPLE.ATTENTION })
    ...
    const aggregate = await FaceStatistic.aggregate([
      { $match: { date: Number(moment(date).format('X')), type: { $in: type }, granularity: CONSTANTS.GRANULARITY.HOUR, summary: CONSTANTS.SUMMARY.MUTILPART, hour: { $lte: time.hours } } },
    ...
    for (const t of type) {
      ...
      if (t === CONSTANTS.PEOPLE.STRANGER) {
        // 如果是陌生人只获取通行系统的陌生人
        count = await Face.countDocuments({ type: t, $and: [{ snapshotTime: { $gte: compareTime } }, { snapshotTime: { $lte: current } }], sys: CONSTANTS.SYS.PASS })
      } else {
        count = await Face.countDocuments({ type: t, $and: [{ snapshotTime: { $gte: compareTime } }, { snapshotTime: { $lte: current } }] })
      }
      face.push({ '_id': t, 'total': count })
    }
    ...
    const statistic = {
      ...
      category: {
        ...
        serise: [{
          name: '访客人数',
          data: [face.some((item) => item._id === CONSTANTS.PEOPLE.VISITOR) ? face.find((item) => item._id === CONSTANTS.PEOPLE.VISITOR).total : 0]
        }, {
          name: '陌生人数',
          data: [face.some((item) => item._id === CONSTANTS.PEOPLE.STRANGER) ? face.find((item) => item._id === CONSTANTS.PEOPLE.STRANGER).total : 0]
        }, {
          name: '员工人数',
          data: [face.some((item) => item._id === CONSTANTS.PEOPLE.SUBJECT) ? face.find((item) => item._id === CONSTANTS.PEOPLE.SUBJECT).total : 0]
        }]
      }
    }
    ctx.status = 200
    ...
    for (const list of Object.keys(collection)) {
      ...
      while (interator <= hour) {
        const result = await FaceStatistic.aggregate([
          { $match: { date: date, granularity: CONSTANTS.GRANULARITY.HOUR, summary: CONSTANTS.SUMMARY.SINGLE, hour: interator, resource: { $in: collection[list] } } },
...
exports.today = async (ctx, next) => {
  ...
  try {
    ...
    const stranger = await Face.countDocuments({ type: CONSTANTS.PEOPLE.STRANGER, sys: CONSTANTS.SYS.CAPUTRUE, $and: [{ snapshotTime: { $gte: compareTime } }, { snapshotTime: { $lte: current } }] })
...
const hotspot = async () => {
  ...
  const date = Number(moment(now.format('YYYYMMDD')).format('X'))
  // 获取0点到现在整点的时间分布
  const hours = Array.apply(null, { length: now.toObject().hours + 1 }).fill(0).map((item, index) => index)
  try {
    for (const hour of hours) {
      ...
      if (hour !== 0) {
        result = await FaceStatistic.aggregate([
          { $match: { date: date, granularity: CONSTANTS.GRANULARITY.HOUR, summary: CONSTANTS.SUMMARY.SINGLE, hour: hour, $or: [{ type: { $in: [CONSTANTS.PEOPLE.COMPARSION] } }, { type: { $in: CONSTANTS.PEOPLE.NORMALSTATIC } }] } },
      ...
      if (result.length > 0) {
        ...
        statistic.push({ name: (hour - 1) + ':00' + '-' + hour + ':00', point: res._id.resourceName, total: res.total, image: '' })
      }
    }
    // 获取人流量最大的八个时间段的位置
...
const park = async () => {
  try {
    ...
    for (const list of Object.keys(collection)) {
      const result = await FaceStatistic.aggregate([
        { $match: { date: Number(moment(now.format('YYYYMMDD')).format('X')), granularity: CONSTANTS.GRANULARITY.HOUR, summary: CONSTANTS.SUMMARY.SINGLE, hour: { $lte: hour }, resource: { $in: collection[list] } } },
      ...
      const count = await Face.countDocuments({ resource: { $in: collection[list] }, $and: [{ snapshotTime: { $gte: compareTime } }, { snapshotTime: { $lte: current } }] })
      // 系统初始化 没有统计数据
      if (!_.isEmpty(result)) {
...
exports.outin = async (ctx, next) => {
  ...
  try {
    ...
    const [time, hour] = moment().format('YYYY-M-D HH').split(' ')
    if (_.isEmpty(resId)) {
    ...
    }).sort({ timepoint: 1 }).lean()
    const label = Array.apply(null, { length: 16 }).fill(0).map((item, index) => (index + 6) + ':00')
    const result = {}
    result.label = label
    result.serise = [
      { name: '进入人数', data: [] },
...
exports.intelligence = async (ctx, next) => {
  ...
  try {
    ...
    const tomorrow = Number(moment(moment().add(1, 'd').format('YYYY-MM-DD')).format('X'))
    ...
    const [tripwire, regionalInvasion, leftObject, rapidMovement, ParkingDetection, peopleGathering, RemoveObject, linger] = await Promise.all([
      AlarmLog.countDocuments({ eventType: 'tripwire', $and: [{ time: { $gte: today } }, { time: { $lt: tomorrow } }] }).exec(),
      AlarmLog.countDocuments({ eventType: 'regionalInvasion', $and: [{ time: { $gte: today } }, { time: { $lt: tomorrow } }] }).exec(),
      AlarmLog.countDocuments({ eventType: 'leftObject', $and: [{ time: { $gte: today } }, { time: { $lt: tomorrow } }] }).exec(),
      AlarmLog.countDocuments({ eventType: 'rapidMovement', $and: [{ time: { $gte: today } }, { time: { $lt: tomorrow } }] }).exec(),
      AlarmLog.countDocuments({ eventType: 'ParkingDetection', $and: [{ time: { $gte: today } }, { time: { $lt: tomorrow } }] }).exec(),
      AlarmLog.countDocuments({ eventType: 'peopleGathering', $and: [{ time: { $gte: today } }, { time: { $lt: tomorrow } }] }).exec(),
      AlarmLog.countDocuments({ eventType: 'RemoveObject', $and: [{ time: { $gte: today } }, { time: { $lt: tomorrow } }] }).exec(),
      AlarmLog.countDocuments({ eventType: 'linger', $and: [{ time: { $gte: today } }, { time: { $lt: tomorrow } }] }).exec()
    ])
    const data = []
...
exports.data = async (ctx, next) => {
  ...
  try {
      ...
      switch (faceType) {
        ...
        case 2:
          ...
          person.push({
            ...
            gender: Math.random() >= 0.300000 ? CONSTANTS.GENDER.MALE : CONSTANTS.GENDER.FEMALE,
            idNumber: randomId(),
        ...
        default:
          person.push({
            resource: resource.id,
            resourcePoint: resource.name,
            gender: Math.random() >= 0.500000 ? CONSTANTS.GENDER.MALE : CONSTANTS.GENDER.FEMALE,
            snapshot: pic,
            snapshotTime: getSnapshotTime(date),
    ...
    await Feature.update({ date: Number(moment(date.format('YYYYMMDD')).format('X')) }, {
      male: male,
      female: female,
      level0: 0,
      level1: 0,
...
const getUserName = () => {
  const family = ['赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '褚', '卫', '蒋', '沈', '韩', '杨', '朱', '秦', '尤', '许', '何', '吕', '施', '张', '孔', '曹', '严', '华', '金', '魏', '陶', '姜', '戚', '谢', '邹', '喻', '柏', '水', '窦', '章', '刘', '李']
  const name = ['观博', '欣竹', '欣阳', '刚军', '扬阳', '靖阳', '熙阳', '嘉萱', '铭阳', '飞', '雨荨', '文博', '诗含', '诗若', '辰海', '晓雨', '展鸣', '晓春', '洪文', '默', '轩杰', '金海', '俊杰', '展旭', '建烁', '婧琪', '婧涵', '诗晴', '传浩', '怡萍', '诗涵', '雅婷', '雅涵', '萍', '晓萍', '兴飞', '小平', '建龙', '宇谟', '子辰', '辰', '湍灵', '骅株', '春莲', '娟敏', '智涵', '欣妍', '慧妍', '雅静', '月婷', '雨婷', '芸馨', '韵涵', '涵韵', '雨欣', '馨蕾', '静媛', '子涵', '雨泽', '静蕾', '茛淯', '珑沧', '芮娟', '梓萱', '轶诚', '嘉文', '晓朋', '一凡', '昊楠', '浩楠', '瑞君', '佳宁', '雨杨', '昊然', '浩然', '滕浩', '雨菡', '海一', '晨宸', '之政', '晨菲', '修闻', '宁夫', '轩', '春菲', '佳涵', '耀宇', '耀雨', '翠', '鑫雨', '涵熙', '继欣', '菲', '兰月', '兰欣', '岚欣', '懿明', '淑菲', '荣凯', '海瑶', '涵雅', '晨曦', '麟炜', '茜', '子萱', '玥菲', '雯菲', '云涵', '靖雯', '馨怡', '江', '运浩', '飞飞', '强', '国馨', '国鑫', '雅雯', '炳君', '海']
...
const randomId = () => {
  ...
  var birthday = '1981' + _.padStart((parseInt(Math.random() * 11) + 1), 2, '0') + _.padStart((parseInt(Math.random() * 25) + 1), 2, '0') // 生日
