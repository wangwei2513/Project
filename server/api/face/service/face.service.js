...
const init = async () => {
  ...
  if (_.isNull(config)) { return false }
  // 获取算法配置
  algorithm = await Algorithm.findOne({ type: config.vender })
  proxyHost(config.ip)
  if (_.isNull(algorithm)) { return false }
  return true
}
// 登录系统
const login = async () => {
  try {
    ...
    await addsystem({ worknum: 104, msg: error.message })
  }
}
// 检测服务器是否可用
const checkOpen = () => {
...
exports.getHistroy = async (params) => {
  try {
    ...
    console.log(error.message)
    await addsystem({ worknum: 103, msg: error.message })
    isLogin = false
    return null
  }
}
