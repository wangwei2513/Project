...
module.exports = () => {
  ...
  setInterval(async () => {
    try {
      ...
      console.log(error)
      await addsystem({ worknum: 102, msg: error.message })
    }
  }, 60000)
  ...
  setInterval(async () => {
    try {
      ...
      console.log(error)
      await addsystem({ worknum: 101, msg: error.message })
    }
  }, 60000)
  ...
  setInterval(async () => {
    ...
    try {
      ...
      if (!_.isNull(result) && !_.isEmpty(result.data)) {
        ...
        // 待插入到人员识别结合中的文档
        const person = []
        const statistics = { male: 0, female: 0, level0: 0, level1: 0, level2: 0, level3: 0, level4: 0, level5: 0, level6: 0, level7: 0, level8: 0, level9: 0 }
        for (const item of result.data) {
          // 获取识别信息对应的资源
          const snapRtsp = _.result(item, 'screen.camera_address')
          const res = resources.find((key) => {
            if (key.resource && key.resource.rtsp) {
              return snapRtsp.indexOf(key.resource.rtsp.main.replace('rtsp://ip:port', '')) >= 0 || snapRtsp.indexOf(key.resource.rtsp.sub.replace('rtsp://ip:port', '')) >= 0
            }
          })
          ...
          if (!item.subject) {
            ...
            if (res.resource.analysisType === CONSTANTS.SYS.CAPUTRUE && !_.isEqual(item.subject.title, '')) {
              ...
              if (Number(item.subject.title) === CONSTANTS.PEOPLE.ATTENTION) {
                ...
                const people = await People.findOne({ type: CONSTANTS.PEOPLE.ATTENTION, status: CONSTANTS.ATTENTION.GOING, username: item.subject.name, visionId: item.subject_id })
                console.log('people', people)
                if (people && people.resources.length !== 0 && people.resources.map(item => item.toString()).includes(res.resource._id.toString())) {
                  alarm.push({
        ...
        if (person.length > 0) {
          await Promise.all([
            ...
            Feature.update({ date: Number(moment(moment().format('YYYYMMDD')).format('X')) }, { $inc: statistics }, { upsert: true })
          ])
        }
        const data = _.dropWhile(person, item => {
          return item.type === CONSTANTS.PEOPLE.SUBJECT || item.type === CONSTANTS.PEOPLE.VIP || item.type === CONSTANTS.PEOPLE.VISITOR || (item.type === CONSTANTS.PEOPLE.STRANGER && item.sys === CONSTANTS.SYS.PASS)
        })
        if (data.length > 0) {
      ...
      console.log(error)
      await addsystem({ worknum: 100, msg: error.message })
    }
  }, 2000)
}
// 人员特征统计
const statistic = (item, stic) => {
  ...
  item.gender >= 0.500000 ? stic.male++ : stic.female++
  stic[level] += 1
}
const getGender = (PR) => {
  return PR >= 0.500000 ? CONSTANTS.GENDER.MALE : CONSTANTS.GENDER.FEMALE
}
