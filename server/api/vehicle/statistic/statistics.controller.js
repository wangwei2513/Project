...
var logUtill = require('../../log/log.controller')
...
exports.index = async (ctx, next) => {
  ...
  if (nowTimeHour === 0) { // 若是凌晨 则减去 一天的时间戳
  ...
  try {
    ...
    // 存储所有资源ID集合
    const lanedatas = await Lane.find({ resource: { $in: rids } }).populate({ path: 'pid', select: 'name _id' }).exec()
    ...
    const lanesDatas = await Lane.find({ passway: 1 }).populate('resource').exec()
    let vcehicleListEntry = []
    if (lanesDatas.length > 0) {
      _(lanesDatas).forEach(n => {
        if (n.resource) { entryChennalIds.push(n.resource.channelid) }
      })
    ...
    logUtill.addsystem(error.message)
  }
}
...
exports.getInOut = async (ctx, next) => {
  try {
    ...
    const inoutresdata = await Lane.find({ passway: { $in: [1, 2] } }, 'passway resource').populate('resource').exec()
    if (inoutresdata.length === 0) { // 若进出口未定义则直接返回0
      ...
      _(inoutresdata).forEach(n => {
        if (n.passway === 1 && n.resource) { inResIds.push(n.resource.channelid) }
        if (n.passway === 2 && n.resource) { outResIds.push(n.resource.channelid) }
      })
      const resultArr = []
      let lastHour = 0
      _(results).forEach(n => {
...
exports.getCrossing = async (ctx, next) => {
  ...
  try {
    if (_.isElement(crossids)) { // 若传入crossid 则查询 否则查询所有入口数据
    ...
    const groupDatas = getGropData(resultArr, 'channelType', 'count', 'name', true, { key: 'channelid', value: chnennlIds })
    ...
    _(sortByDatas).forEach((n, num) => { // 查询当前最大的路口channelid集合
      if (num < maxCrossNum) {
    ...
    _(mapDatas).forEach(n => {
      ...
      const tempMapArr = []
      _(n).forEach(x => {
        if (_.indexOf(sortByChannelIds, x.channelid) >= 0) { // 若id存在 上面存在的id 的话  与路口保持一致
          tempMapArr.push(x)
        }
      })
      if (n.length > 0) { // 若有数据才进行统计
...
exports.getDefense = async (ctx, next) => {
  ...
  try {
    ...
    _(DSs).forEach(n => {
      n = n.toObject()
      if (CCObj[n.channelid]) { n['crossName'] = CCObj[n.channelid] } // 添加路口名称
      _DSs.push(n) // 保存到副本_DSs
    })
    const gropDatas = _.groupBy(_DSs, 'date') // 按照日期分类
    const resultArr = []
    for (const gd in gropDatas) { // 按照日期分类统计
...
exports.getDefenselist = async (ctx, next) => {
  const searchObj = await getDefenseSearch(ctx)
  try {
    const result = await paging.listQuery(Defenserecord, searchObj, 'defenseType createTime channelid', {}, ctx.query.page, '', ctx)
...
exports.getCurMaxZone = async (ctx, next) => {
  ...
  try {
    const channelDatas = await Statistics.find({ date: date }, 'channelType hour').populate('channelType').exec()
    ...
    ctx.body = _.sortBy(dataArr, 'count').reverse().slice(0, 7) // 排序拿到前七条数据
  } catch (error) {
    return ctx.throw(500, { code: 2101, message: error.message })
  }
}
...
exports.getSomeCount = async (ctx, next) => {
  ...
  try {
    ...
    const tempObj = {}
    let countData = 0
    _(discoveryDefenseData).forEach(n => { // 去重求和
    ...
    const inoutresdata = await Lane.find({ passway: { $in: [1, 2] } }, 'passway resource').populate('resource').exec()
    // inoutresdata = []
    if (inoutresdata.length === 0) { // 若未指定进出口车道则直接返回0
      resultObj['inCount'] = 0
      ...
      if (_.isEmpty(tempArr)) { // 若为空 则进行复制
        resultObj['inCount'] = 0
        resultObj['outCount'] = 0
      }
      _(tempArr).forEach(n => {
        if (inoutChannelIds.indexOf(n.channelid) > -1) {
          if (inOutDict[n.channelid] === 1) { // 判断是否入口
            if (resultObj['inCount']) {
              resultObj['inCount'] += n.count
            } else {
              resultObj['inCount'] = n.count
            }
          } else if (inOutDict[n.channelid] === 2) { // 判断是否出口
...
const getSomedayData = async (searchObj, type, isCount) => {
  ...
  const tempArr = []
  for (const gd in gropData) { // 遍历对象单个调用 type分类统计
    const tempObj = {}
    ...
    if (st.length > 0) { tempArr.push(tempObj) }
  }
  return tempArr
}
    ...
    _(gropData[y]).forEach(x => {
      if (!_.isEmpty(x[property])) {
        _(x[property]).forEach(n => {
          ...
          if (tempObj[n[orderBy]] && flag && isSum) { // 判断对象是否存在，若不存在初始化数据，若存在则累加
...
const getChnennlIdsByCrossingIds = async (crossids = []) => {
  const searchObj = {}
  if (crossids.length > 0) { searchObj.pid = { $in: crossids } }
  const laneDatas = await Lane.find(searchObj).populate({ path: 'resource', select: 'channelid' }).exec()
  const chnennlIds = []
  _(laneDatas).forEach(n => {
...
const getCrossingNameByChennalIds = async (chIds) => {
  ...
  const laneDatas = await Lane.find({ resource: { $in: ridsArr } }, 'pid resource').populate({ path: 'pid', select: 'name' }).exec()
  const CCObj = {} // 频道路口名称 字典
  _(laneDatas).forEach(n => {
    if (n.pid) { CCObj[RCObje[n.resource]] = n.pid.name }
  })
  return CCObj
}
...
const getCrossingVehicleTypeOneDay = async (crossids, date) => {
  ...
  if (crossids.length > 0) { // 若传入crossid 则查询 否则查询所有入口数据
    const resultCrossing = await Crossing.find({}, '_id').exec()
    _(resultCrossing).forEach(n => {
      crossids.push(n._id)
    })
...
const getDefenseSearch = async (ctx) => {
  ...
  if (defenseType !== 'all') { searchObj['defenseType'] = defenseType }
  if (compareType === '1') {
    searchObj['date'] = { $gte: startTime, $lte: endTime }
  } else {
    searchObj['date'] = { $in: [startTime, endTime] }
  ...
  const laneDatas = await Lane.find(laneSearchObj, 'resource').populate({ path: 'resource', select: 'channelid' }).exec()
